...
| **3D-P2 — CANONICAL (OPEN)** | [3d-p2-instability-investigation.md](mms/3d-p2-instability-investigation.md) | **2026-07-05 (authoritative): root cause OPEN — a Gridap↔paper implementation discrepancy that c₁ MASKS.** P2-3D is "converged-but-wrong" in Gridap at paper c₁, and c₁×4 collapses L²u ~40× (ratio-to-interpolant → 1) — but **paper c₁=4k⁴ is CORRECT**: the paper's first author confirms **Kratos runs the FULL subscale at paper c₁ on tets and solves this exact case optimally** (ASGS & OSGS). So the "element-family c₁ coercivity deficit" reading is **REFUTED**; c₁ is a symptom-mover, not the cause. NOT mesh quality / grad-div / inf-sup / Jacobian either (all refuted with numbers, §3). **Root cause OPEN** — most likely (not certainly) the P2-3D viscous 2nd-derivative subscale; may be broader. NumPy clean-room: [convergence_problems_audit/](convergence_problems_audit/) (verdict refuted — it inherited the discrepancy). OSGS-P2 has a separate ∂π/∂u solver issue (solution accurate but `ok=false`). |
