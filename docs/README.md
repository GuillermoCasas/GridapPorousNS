...
| **3D-P2 — CANONICAL (RESOLVED)** | [3d-p2-instability-investigation.md](mms/3d-p2-instability-investigation.md) | **2026-07-06 RESOLVED: NOT a bug/discrepancy — `4k⁴` is under-margined for high-`C_inv` structured tets.** The viscous 2nd-derivative subscale is anti-coercive by construction (self-adjoint op ⇒ `B_S` has `−τ‖𝓛_visc V‖²`), dominated by `c₁ > 2ξC_inv²`; `4k⁴` has ~zero margin for 3D tets (`C_inv²` Kuhn 214 vs quad 60, mesh-indep; Kuhn shape-reg 8.36 worst) → structured Kuhn is over the coercivity edge. Proven by Céa (50× error ⇒ coercivity≈0, refutes definite-yet-buggy) + the `c1_mult=4` monotone convergent ladder. Reconciles with the paper (unstructured tets, 3 meshes, line 910 element-dependent c₁). The 2026-07-05 "Gridap↔paper discrepancy/c₁-masks-a-bug" framing is WITHDRAWN. Refuted alternatives (sign, h-convention, code factor, Newton-vs-Picard, simplex, anisotropy) recorded in §3/§3.2. OSGS-P2 has a separate ∂π/∂u solver issue (accurate but `ok=false`). |
| **3D-P2 — dossier (scrutiny)** | [3d-p2-coercivity-resolution-dossier.md](mms/3d-p2-coercivity-resolution-dossier.md) | **Detailed for-scrutiny record:** every experiment + numbers (C_inv table, mesh-independence, shape-regularity, c₁×4 ladder, τ/h levers, BCC attempts), the full argument, the refuted-alternatives table (A–O), the paper reconciliation, and the **honest caveats + what would overturn the verdict** (the absolute `ξ` constant, the un-demonstrated well-shaped-tet positive test, the deviatoric-specific sharper root). |
| 3D penalty + OSGS coupling | [3d-iterative-penalty-fix-and-osgs-coupling.md](mms/3d-iterative-penalty-fix-and-osgs-coupling.md) | Canonical for the **iterative-penalty (well-posedness) fix** — 3D all-Dirichlet is ill-posed at ε=0; adds `ε_num·(pⁿ−pⁿ⁻¹)` to the mass residual — plus the eps_pert homotopy and the OSGS **∂π/∂u** coupling problem. Its "P2 root cause = penalty" claim is withdrawn (penalty fixes *well-posedness*, not the P2 *accuracy* defect); and its *original* "NOT c₁" instinct is **VINDICATED (2026-07-05)** — paper c₁ is correct, the P2 defect is an OPEN Gridap↔paper discrepancy (see canonical above), neither penalty nor c₁. |
| 3D ASGS (P₁ & P₂) — superseded | [3d-p2-convergence-investigation.md](mms/3d-p2-convergence-investigation.md) | Superseded as canonical, and its **c₁ verdict is now REFUTED (2026-07-05)** — both its original "c₁ under-budgets coercivity for 3D tets" and the 2026-07-03 "RE-CONFIRMED" restatement are overturned (paper c₁ is correct per the first author / Kratos full-terms; c₁ masks a Gridap↔paper discrepancy; root cause OPEN). Still-valid unique content: §4 (iteration/Jacobian/orchestration exonerated) + §6 (OSGS-vs-high-Da reconciliation); also documents the genuine **3D P1-ASGS** L²-order sub-optimality (OSGS recovers it). |
