...
Last updated: 2026-06-02 (caveats below carry their own dates; reviewed in the 2026-06-04 documentation audit). This is the **canonical** MMS reference; `mms_fold_recovery.md` is its fold sub-topic and `mms_next_actions.md` is a dated action snapshot.
...
> 4. **(2026-06-02) Encoding-dependence supersedes caveat #2 for low-Re/high-Da OSGS cells, and "Da is benign" (§ table below) is OSGS-false.** A controlled A/B (`centered` vs `minmax` encoding, only that knob varied) on **Re=1, Da=1e6, α=1, OSGS** shows `centered` converges **optimally** (L²(u) rate ≈2.0, H¹(u) ≈1.0) while `minmax` (this session's new default) **stagnates** (L²(u) 0.89, H¹(u) 0.17). Same dimensionless problem, same eps_pert=1.0. The cause (pinned by trace instrumentation; see [`lessons_learned.md`](../lessons_learned.md) 2026-06-02 + [`mms_next_actions.md`](next-actions.md) §3) is a **scale-coupled inner-Newton gate × a 1-iteration inner cap** — NOT the τ₁ balance (unit-invariant) and NOT floating point. The per-field gate `‖R_k‖≤rel·‖x_k‖` has `eff_ftol∝‖x‖∝U` (measured 33× between encodings); combined with `osgs_inner_newton_iters≈1`, centered's inner solves fire `initial_ftol` (projection below gate → OSGS≡ASGS → optimal) while minmax's fire `max_iters_stagnation` (one Newton step lands ~1e4× above gate → under-converged states → outer drift floors at ~1e-4 → stagnant). The ~1e-4 floor is a single-step/under-convergence level (~12 orders above machine ε), not roundoff. **Outcome (2026-06-02):** the *covariance* half of this (the OSGS answer changing with the encoding) was fixed by freezing the per-field gate at the initial residual `‖R₀‖` and making the warmup tolerance a dimensionless relative-reduction target ([`lessons_learned.md`](../lessons_learned.md) 2026-06-02, guarded by `encoding_invariance_test.jl`). The *rate*-stagnation half (high-Da OSGS, Re=1/Da=1e6) is **separate and still open** — the proposed "lift the inner cap + scale-invariant gates" cure was tested 2026-06-02 and did **not** fix it; the remaining defect is in the discrete staggered map, not the gates. The "larger σ (high Da) is a coercive, stabilizing term — not a trouble source" claim below held only for the (then-mislabelled) ASGS data; for OSGS, high Da interacts with the encoding. Full record: [`lessons_learned.md`](../lessons_learned.md) 2026-06-02 entries. **Dated action snapshot: [`mms_next_actions.md`](next-actions.md).**
> 5. **(2026-06-04) The open rate-stagnation half of caveat #4 is now RESOLVED-as-confirmed by the complete sweep.** The full k=1 sweep (288/288, N=10→320) was re-run on the **fully-covariant** code (covariant inner gate `‖R₀‖` + covariant *relative* warmup + covariant `eps_val`; `minmax` encoding) — snapshot `test/extended/ManufacturedSolutions/previous_results/_archive_postFix_covariant_complete/phase1_quad_k1.h5`. With covariance removed as a confound, the OSGS rate-stagnation **persists exactly where the staggered-map hypothesis predicted**, and is now sharply scoped:
> &nbsp;&nbsp;&nbsp;&nbsp;• **Only Da=1e6 with Re ∈ {1e-6, 1}**: OSGS velocity H¹ rate **0.62–0.83** (vs ASGS ~1.1–2.1), L² **1.59–1.78** (vs ASGS ~1.9–2.65).
> &nbsp;&nbsp;&nbsp;&nbsp;• **Re=1e6, same Da=1e6 → OSGS healthy** (2.0/1.0). For Da ∈ {1e-6, 1}, OSGS matches/beats ASGS everywhere.
> &nbsp;&nbsp;&nbsp;&nbsp;• **α₀-independent**: just as severe at **α₀=1** (1.59/0.62, no porosity layer) as at α₀=0.05 — so it is *pure reaction-dominance*, a different axis from the high-Re/low-α₀ fold of caveats #2/#3.
> &nbsp;&nbsp;&nbsp;&nbsp;This retires the "is it the gates or the map?" question: **it is the discrete staggered map** `π ↦ Π(R(U(π)))` in the reaction-dominated regime — convection (high Re) restores its contraction, pure reaction dominance does not. Every stagnating cell logs *"Solver converged but MMS plateau was NOT formally verified (budget exhausted)"*. Tracked as the open numerical defect in [`../known_issues.md`](../known_issues.md); efficiency-side context in [`../solver/efficiency-ideas.md`](../solver/efficiency-ideas.md) Idea 4.
>
> &nbsp;&nbsp;&nbsp;&nbsp;6. **(2026-06-05) `freeze_after_k` does NOT fix or mask this — it shows it honestly.** The `osgs_projection_coupling="freeze_after_k"` mode ([`../solver/efficiency-ideas.md`](../solver/efficiency-ideas.md) Idea 5) does k projection-updating warm-up steps then freezes π for a quadratic finish, and **always applies the projection (no ASGS fallback** — an interim H¹-drift gate that reverted to ASGS here was removed at the user's direction). So in this corner `freeze_after_k` converges toward the *same* sub-optimal OSGS fixed point and these cells **display OSGS's own poor/negative H¹ rate** (error growing with refinement), rather than the optimal ASGS answer. The formulation defect in the OSGS fixed point itself remains open and unfixed; `freeze_after_k` only changes *how fast* you reach that (defective) fixed point, not the rate — exactly as the rate-vs-constant analysis predicts.
>
> &nbsp;&nbsp;&nbsp;&nbsp;7. **(2026-06-05) Fixed-point nature confirmed by a `staggered` vs `coupled` control; "defect" reframed as the coercivity gap; rate question reopened.** A clean A/B at Re=1, Da=1e6, **α₀=1** (no porosity layer), N=10→320, varying only `osgs_projection_coupling`, shows **OSGS-coupled ≡ OSGS-staggered to ~4 sig figs across 5+ meshes** — `coupled` converging *gracefully* to `ftol` (no budget exhaustion). This confirms caveat #6's "it is the fixed point" but corrects the *cause*: not a staggered-map pathology but the genuine coercivity gap of [`../../theory/osgs_reaction_note.tex`](../../theory/osgs_reaction_note.tex) Prop. 1 (degrades with Da_h, recovers at Re=1e6). The reaction-projection trim is verified correct and **exonerated**. The OSGS H¹ rate is sub-optimal on every tested mesh but **creeps upward (0.57→0.74)** — so "asymptotic order reduction" vs "very slow pre-asymptotic climb" (which would vindicate the note's optimal-rate claim) is **open**; a fine ladder N=640/1280 settles it. Canonical write-up + options + the `freeze_after_k` recommendation: [`../solver/osgs-reaction-dominated-rate.md`](../solver/osgs-reaction-dominated-rate.md).
