...
(`4k+4`). Inserted as reference rows in `tab:3D`. At the tables' **adopted `c₁=16k⁴`**,
`α₀=0.5`:
...
**Verdict: the paper's entire a priori chain — for BOTH stabilizations — is machine-checked.** For **ASGS**, `lemma:Stability`, `lemma:Continuity`, `lem:continterp` and `thm:convergence` are complete Coq theorems. For **OSGS**, the linearized chain (`th:StabilityOSGS` inf–sup stability, interpolation continuity, consistency, `th:ConvergenceOSGS`) is now **also** a complete Coq development — `abstract_osgs_stability` / `abstract_osgs_continterp` / `abstract_osgs_consistency` / `abstract_osgs_convergence`, in `Osgs*.v` + the `NonVacuityOsgs.v` witness (added alongside the article_v2 App-D integration). The tree is `proof_verification/coq-formal/`, now **24 `.v` files**, with **0 `Admitted` / 0 `Axiom`**, all re-verified by `coqchk`. `Print Assumptions` on each ASGS headline returns exactly three standard-library axioms and **no user axiom**: two from the classical-reals construction (`ClassicalDedekindReals.sig_not_dec`, `sig_forall_dec`) plus functional extensionality (`FunctionalExtensionality.functional_extensionality_dep`). The tree verifies under **Rocq 9.1.1** in this environment (CLAUDE.md pins Coq 8.18; the only difference is a `From Coq`→`From Stdlib` deprecation warning). Full map (ASGS = App. C, OSGS = App. D of `article_v2.tex`) and the ASGS 50-row hypothesis inventory: [`proof_verification/coq_coverage.tex`](../proof_verification/coq_coverage.tex).
...
### Audit-response reruns — results (2026-07-21/22)
...
- **R5 / D05 — stabilized Taylor–Hood control.** Applying the ASGS residual stabilization to the *same* inf-sup-stable P2/P1 pair as the unstabilized-Galerkin baseline: it **converges at Re=10⁵ exactly where the unstabilized TH velocity stagnates** (n.c.). **Settled on the completed `N=320` ladder (§7h, 2026-07-31)**, the recovery is partial and norm/porosity-dependent: the pressure attains its bound rate at both porosities (`2.00`/`2.53` in `L²`, `1.00` at both in `H¹`), the `L²` velocity is optimal at α₀=0.1 (`3.00`) but not at α₀=0.5 (`2.36`), and the `H¹` velocity falls short at both (`1.32`, `1.65` against 2). Those shortfalls are **stable under refinement** — the α₀=0.5 pair read `2.41`/`1.26` over the previous doubling — so they are a property of the stabilized pair in this regime, not pre-asymptotic. In the viscous regime the stabilization costs accuracy — a redundant perturbation on an already-stable pair — by **1.2×** (α₀=0.5) to **1.4×** (α₀=0.1) at the common finest mesh `N=320`, *not* the ~10× first recorded here, which compared the stabilized row's `N=160` FME against the unstabilized row's `N=320` (see [[lessons_learned]] 2026-07-30 (b)). This **isolates the high-Re gain to the stabilization, not the space pair** — the decisive answer to D05. Added as the `$\mathbb{P}_2/\mathbb{P}_1$ ASGS` rows in `tab:CocquetMMS`. Config `cocquet_form_mms_taylorhood_stabilized.json`.
- **R6 / N19 — genuinely-3D field.** `u = U α₀·curl(A)/α` (all components x,y,z-dependent, u_z≠0, div(αu)=0 exactly). On the same regular Kuhn family at c₁=16k⁴, the method attains **optimal rates for both orders and both stabilizations** (P1: L2u≈1.9/H1u≈1.0; P2: L2u≈3.5/H1u≈2.3). Notably the **OSGS pressure converges in the H¹-seminorm** (P2 slope 2.0), *unlike* the extruded field's saturation (~1.29, slope≈0) — so the extruded pressure *rate* plateau was tied to the z-invariant degeneracy. Note the sharper reading the paper adopts: the absolute `H¹` pressure errors stay `O(1)` (1.22–2.97) on the genuine-3D field too, so the near-stagnant **magnitude** is **not** an artifact of the degeneracy — only the rate recovers. (Consistent with the R10 reading, not contradicting it.) Exact Newton reaches ‖R‖=3.5×10⁻¹⁶, confirming the hand-derived forcing is exact end-to-end. Table `theory/paper/genuine3d_table.tex`, now `\input` after `tab:3D` in `article_v2.tex` as `tab:Genuine3D` (decision: add-alongside); harness `smoke3d.jl sweep_genuine3d` → `results/k*/TET/genuine3d/`.
