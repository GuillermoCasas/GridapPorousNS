...
function check_alpha()
    ...
    # Override alpha locally
    # alpha_func(x) = 1.0
    # Gridap.∇(::typeof(alpha_func)) = x -> VectorValue(0.0, 0.0)

    model, X, Y, dΩ, h_cf, alpha_h, refe_u, refe_p, local_config = build_solver(20, base_config_dict, Re, c_in)
    
    # Force alpha to 1.0 exactly
    alpha_h = interpolate_everywhere(1.0, X.spaces[2])

    solver_picard, solver_newton, c_1, c_2 = build_nonlinear_solver(local_config, 2)
    form = PorousNSSolver.PaperGeneralFormulation()

    x0 = (interpolate_everywhere(VectorValue(0.0, 0.0), X.spaces[1]), interpolate_everywhere(0.0, X.spaces[2]))
    ...
    println("Alpha=1 Success: ", sys_success, " iterations: ", sys_iter_count)
end
