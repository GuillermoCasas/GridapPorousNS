...
\date{Revised 2026-07-12 \quad (14 files, $\approx$531 theorems;
      developed on Coq 8.18; all 14 files build-verified on Rocq 9.1.1)}
...
quantitative} trusted base of standard analytic facts; the convergence
...
\textbf{14 files, $\approx 531$ theorems, all proved} --- there is no
...
\emph{only} the standard library, and all fourteen files compile and
kernel-check unchanged under the current Rocq~9.1.1 (the two new files use
no constructs beyond those of the existing suite).
...
\end{itemize}
Neither appears anywhere in the fourteen files. Consequently every result below
is reduced to the standard-library axiomatisation of the real numbers (plus,
for the four abstract theorems, the explicitly-listed analytic hypotheses ---
...
Dependency and topic map of the fourteen files. \file{Limits.v}/\file{DerivKit.v}
...
prior files are \coq{HBS_W} (the tested identity for the discrete-error
pair --- the same class-G item as the stability lemma's own \coq{HBS}) and
\coq{Horth} (Galerkin orthogonality \lbl{eq:consistency} together with
bilinearity of $B_S$ in its first argument).
...
\coq{HBS_W} & the tested identity \lbl{eq:StabilityEstimate} for the discrete-error pair (same class as \coq{HBS}) & G & hand-audited \\
...
The practical consequence: for every theorem in these fourteen files, the claim
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{The hypotheses holding in the application.} For the two central
    lemmas, a short list of standard analytic facts (the trusted base of
    \S\ref{sec:left}) is \emph{assumed}, not proved here. These are textbook
    results, hand-audited against the manuscript --- but ``hand-audited'' means
    human-checked, so they still carry the ordinary, small-but-nonzero risk of
    any careful proof.
...
\midrule \endhead
\textbf{1. Certain \newline (unconditional; positivity only)} &
All the scalar / algebraic content: the $\tau_1,\tau_2,\sigt$ identities, the
coefficient expansions and lower bounds, the $\varepsilon$-smallness chain, the
parameter and jump inequalities, the Fourier and coupling spectra, the
asymptotic limits, the manufactured-solution divergence identity, and the
plateau bump's $C^1$ behaviour. &
Only the trust base (kernel $+$ real-number axioms). The positivity hypotheses
are physically trivial. \\
\midrule
...
Trust base, \emph{plus} the dozen named analytic hypotheses of \S\ref{sec:left}
(Green-type identities, inverse estimates) holding --- standard, hand-audited,
Lean-targeted. \\
\midrule
...
\begin{itemize}[nosep]
  ...
  \item \textbf{The two consistency-class items of the convergence file}
    (\coq{HBS_W}, \coq{Horth}): Green-type material of the same character
    as \coq{HBS}, hand-audited; the interpolation estimates \coq{HI_*}
    join the Class-I Lean targets.
...
verified to a standard of certainty that human refereeing cannot reach. What
still rests on human judgement is a \emph{short, explicit, named} list: a
handful of standard analysis facts (Green-type identities, inverse and
interpolation estimates --- being formalised next in Lean) and the physical-
and code-modelling questions that no symbolic proof can settle. The value of the
...
\noindent
All fourteen files compile and kernel-check unchanged on Rocq~9.1.1 (the only
