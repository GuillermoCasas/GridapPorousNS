...
The practical consequence: for every theorem in these twelve files, the claim
...
\lbl{lemma:Stability} and \lbl{lemma:Continuity}: the coercivity bound
$B_S\ge\Cstab\,|||U_h|||^2$ and the continuity bound
$B_S\le C_{\mathrm{tot}}(\mathrm{Br}_U+\mathrm{Br}_P)|||V|||$, with explicit
constants and every intermediate step (Cauchy--Schwarz, Young, the
eighteen-term bookkeeping, the jump treatment, the summation over the mesh)
inside the kernel. &
...
Lean-targeted. \\
\midrule
\textbf{3. Hand-audited only \newline (no kernel check yet)} &
The interpolation replacements \lbl{lem:continterp} and the convergence theorem
\lbl{th:Convergence} built on them; the Appendix~A elemental-matrix
transcription (left to SymPy); the subscale-norm / CDR-adjoint items outside
stdlib-Coq scope. &
The ordinary risk of a careful human proof. Next to be formalised. \\
\bottomrule
\end{longtable}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Interpolation and convergence} (\lbl{lem:continterp},
    \lbl{th:Convergence}): the one corner of the stability/continuity argument
    with no Coq statement at all --- hand-audited only.
...
stability and continuity proofs --- are now verified to a standard of certainty
that human refereeing cannot reach. What still rests on human judgement is a
\emph{short, explicit, named} list: a handful of standard analysis facts (being
formalised next in Lean), one interpolation step, and the physical- and
