# test/extended/CocquetAlpha1/run_convergence.jl
# ==============================================================================================
# Nature & Intent (α≡1 control):
# Identical to test/extended/CocquetExperiment/run_convergence.jl — SAME geometry, SAME mixed BC
# (inlet+walls Dirichlet, traction-free Neumann outlet), SAME self-reference (N=200) convergence
# methodology — EXCEPT the porosity is set to α≡1 everywhere (see alpha_func below).
#
# Purpose: with α≡1 the generalized Darcy–Brinkman–Forchheimer formulation collapses to plain
# incompressible Navier–Stokes (no porosity weighting, reaction σ≡0, porosity interpolation exact).
# If THIS converges optimally (O(h^{k+1})) on the same mixed-BC geometry, then the core NS solver
# and the mixed-BC/corner are sound, and the benchmark's P2/P2 underperformance is localized to the
# GENERALIZED formulation (porosity-dependent reaction / porosity-weighted operators / interpolated
# porosity). If α≡1 is ALSO capped, the mixed-BC corner is implicated independently of porosity.
#
# Associated Data & Endpoints:
# - `data/alpha1.json`: physical scales, geometry, Re, c_in, element pairs (porosity profile is
#   overridden in code to α≡1, so the config's porosity fields are inert here).
...
# α ≡ 1 EVERYWHERE (the ONLY change vs CocquetExperiment).
# This collapses the generalized formulation to plain incompressible Navier–Stokes: every
# α-weighted term reduces to its standard form, and the Forchheimer reaction vanishes identically
# (σ = σ_lin·((1-α)/α)² + σ_nl·((1-α)/α)|u| = 0 at α=1). The constant 1 is reproduced exactly by
# the FE interpolant at any order, so the porosity-interpolation error is also zero. Everything
# else — geometry, mixed BC (traction outlet), self-reference methodology — is identical.
alpha_func(x) = 1.0
Gridap.∇(::typeof(alpha_func)) = x -> VectorValue(0.0, 0.0)
...
function build_solver(N::Int, config_dict, Re::Float64, c_in::Float64)
    ...
    # τ characteristic length: √(2·area)=1/N for TRI, matching CocquetExperiment/CocquetFormMMS (rate-neutral fix).
    h_array = local_config.numerical_method.mesh.element_type == "TRI" ?
...
function run_convergence()
    ...
    # Config filename may be passed as the first CLI argument (default: alpha1.json,
    # the α≡1 control; equal-order P1/P1 + P2/P2).
    config_file = length(ARGS) > 0 ? ARGS[1] : "alpha1.json"
    ...
    for ep in element_pairs
      ...
      for method in methods
            ...
            for N in N_list
                ...
                # Free UMFPACK C-pointer memory from the solver to prevent OOM / SIGILL
                GC.gc()
            end
