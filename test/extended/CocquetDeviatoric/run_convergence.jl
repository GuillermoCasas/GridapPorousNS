# test/extended/CocquetDeviatoric/run_convergence.jl
# ==============================================================================================
# Nature & Intent (viscous-operator flip):
# Identical to test/extended/CocquetExperiment/run_convergence.jl — SAME varying porosity
# ε(y)=0.45+0.55·exp(y-1) interpolated at order k_v, SAME Forchheimer reaction, SAME mixed BC
# (inlet+walls Dirichlet, traction-free Neumann outlet), SAME self-reference (N=200) methodology —
# EXCEPT the viscous operator is DeviatoricSymmetric (the manufactured-solutions / canonical
# operator) instead of Cocquet's SymmetricGradient. The swap lives entirely in data/deviatoric.json
# (viscous_operator_type), which build_formulation reads; no code change is needed here.
#
# Purpose: the α≡1 control showed plain Navier-Stokes is ALSO P2/P2-capped on this mixed-BC
# geometry, implicating the Dirichlet/Neumann outlet corner. This test asks whether that cap is
# specific to the SymmetricGradient operator's natural outlet traction (2νS(u)-pI)·n=0, or is
# operator-independent. If P2/P2 recovers under DeviatoricSymmetric → the operator/its outlet
# traction is the cause; if still capped → the mixed-BC corner is operator-independent.
#
# Associated Data & Endpoints:
# - `data/deviatoric.json`: physical scales, geometry, Re, c_in, element pairs; viscous_operator_type=DeviatoricSymmetric.
...
function run_convergence()
    ...
    # Config filename may be passed as the first CLI argument (default: deviatoric.json,
    # the DeviatoricSymmetric viscous-operator flip; equal-order P1/P1 + P2/P2).
    config_file = length(ARGS) > 0 ? ARGS[1] : "deviatoric.json"
