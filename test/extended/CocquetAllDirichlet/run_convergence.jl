# test/extended/CocquetAllDirichlet/run_convergence.jl
# ==============================================================================================
# Nature & Intent (FLIP B — all-Dirichlet diagnostic):
...
# EXCEPT that the outlet is a Dirichlet boundary (see build_solver) rather than the traction-free
# Neumann outlet of the benchmark.
#
# Purpose: isolate the cause of the benchmark's P2/P2 velocity-L² underperformance. The benchmark's
# mixed Dirichlet(wall)/Neumann(traction outlet) corners are candidate singularities that cap
# high-order convergence. Imposing a smooth, corner-compatible Dirichlet profile on the outlet
# removes that incompatibility. If P2/P2 recovers toward O(h^{k+1}) here, the mixed-BC corner was
# the cause; if it stays capped (~1.3), the interpolated porosity (Flip A) is implicated instead.
#
# There is NO traction outlet to truncate, so outlet_truncation_delta is 0 (errors over the whole
# domain). All other knobs come from data/all_dirichlet.json.
#
# Associated Data & Endpoints:
# - `data/all_dirichlet.json`: physical scales, geometry, Re, c_in, element pairs.
...
function build_solver(N::Int, config_dict, Re::Float64, c_in::Float64)
    ...
    # FLIP B (vs CocquetExperiment): the outlet is ALSO a Dirichlet boundary, carrying the
    # SAME parabolic profile as the inlet. This is the only structural difference from the
    # benchmark — it converts the wall/outlet corners from Dirichlet–Neumann (mixed, singular)
    # to Dirichlet–Dirichlet (compatible). The inlet parabola c_in·y(1-y) vanishes at y=0,1 so
    # it matches the no-slip walls at the corners, and it is flux-balanced because the porosity
    # depends only on y (∫_inlet αu·n + ∫_outlet αu·n = 0 exactly).
    ...
    # [DIAGNOSTIC] quadrature-adequacy test: +12 over the nominal rule. Revert after.
    dΩ = Measure(Ω, degree + 12)
...
function run_convergence()
    ...
    # Config filename may be passed as the first CLI argument (default: all_dirichlet.json,
    # the all-Dirichlet Flip-B equal-order P1/P1 + P2/P2 study).
    config_file = length(ARGS) > 0 ? ARGS[1] : "all_dirichlet.json"
