...
3. **External Homotopy Dilution**: If terminal noise floors physically restrict roots natively, the MMS test harness descends a *perturbed-to-clean* homotopy ladder $eps\_pert = 1.0 \to 0.1 \to \dots \to 0$, starting Newton from a heavily perturbed initial guess and only falling back to a cleaner one (down to $eps\_pert = 0$, i.e. the exact solution interpolated onto the FE mesh) if Newton fails. The first attempt that converges is recorded as that mesh's `eval_eps`; that column is an explicit per-cell measurement of where each solver needed help.
