...
the differential operator, the viscous form, and the reaction law and its
coefficients are identical to the reference. The \emph{discretization} is not:
this is an \emph{operator-matched stabilized diagnostic}, in which the
reference's element pair is combined with our ASGS residual stabilization,
whereas the cited method is the pure Galerkin restriction on that pair (see
\texttt{cocquet\_formulation.tex}, \S``Discretization''). The word
``reproduction'' is reserved for the unstabilized Galerkin Taylor--Hood run of
\texttt{CocquetTubeTest}. Pure Navier--Stokes
...
Taylor--Hood $\mathbb{P}_2/\mathbb{P}_1$ elements on simplicial (TRI)
meshes---the reference's element pair---with our ASGS stabilization active,
which the reference does not use. The porosity is interpolated to the
...
# NB: run_test.jl prepends "data/" itself -- pass the BARE filename.
# pure Galerkin (the reference method):
julia --project=../../.. run_test.jl cocquet_form_mms_taylorhood.json
# ASGS on the same pair (this diagnostic):
julia --project=../../.. run_test.jl \
      cocquet_form_mms_taylorhood_stabilized.json
# rates stored as HDF5 attributes rate_u_l2, rate_p_l2, ... in
#   results/cocquet_form_mms_taylorhood.h5
#   results/cocquet_form_mms_taylorhood_stabilized.h5
\end{verbatim}
