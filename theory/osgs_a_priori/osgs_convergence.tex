% =============================================================================
% Working note, superseded as a submission source by Appendix D of
% theory/paper/article_v2.tex (osgs_appendix.tex / osgs_appendix_commented.tex).
%
% 2026-07-29: the article now states the properties of the viscous projector as a
% standing assumption -- a pointwise, constant-coefficient orthogonal projection that
% is Korn-compatible on H^1_0 -- and proves that the four operators {I, sym, dev,
% dev-sym} are instances.  Everything below holds verbatim for any such operator; the
% only place the deviatoric-symmetric form is used is the definiteness lemma, whose
% inline identity is the dev-sym instance of the general chain.  See Sec. 2.1 of the
% article (subsection "The viscous projector", sec:ViscousProjector) -- until
% 2026-07-29 this material sat in an Appendix C subsubsection.
% =============================================================================
...
plus the local mesh Damk\"ohler number; the theorem is in fact established in the sharper
porosity-weighted broken $\ell^2$ functional of which this is the $\ell^1$ majorant.
The factor is asymptotically harmless
...
\begin{equation}
  ...
  \tnorm{U - U_h}
  \;\le\;
  C \Bigl( \sum_{K} \bigl(c_1 + \Dah\bigr)\,\frac{\alpha_K^2}{h_K^2}
  ...
  \Bigl( \tau_{2,K}^{1/2}\,\Eint{K}{\bu} + \tau_{1,K}^{1/2}\,\Eint{K}{p} \Bigr),
\end{equation}
the sharp left-hand functional being the porosity-weighted broken $\ell^2$
quantity $\Psi_{\mathrm O}(h)$ of \cref{eq:ErrorFunctionL2} and the right-hand one its
coarser $\ell^1$ majorant $\mathcal{E}(h)$ of \cref{eq:ErrorFunction},
...
and the best conversion available to the present proof costs a factor $\Dah^{1/2}$. Both sides of this
...
of $L^2(\Omega)$ (for $\varepsilon > 0$ the zero mean of the solution is automatic---integrating
the mass equation $\varepsilon p + \nabla\cdot(\alpha\bu) = 0$ over $\Omega$ and using
$\bu \in V_0$ gives $\varepsilon\int_\Omega p = 0$---so whether or not the constraint is imposed
on the space is immaterial to what follows). Set $\mathcal{X}_0 \coloneqq V_0\times Q_0$ and, for
$U = [\bu; p]$, $V = [\bv; q]$,
\begin{equation}
...
\begin{equation}
  ...
  + \bigl\lVert\Poneo \xi_h\bigr\rVert_{\tau_1}^2
  + \bigl\lVert\Ptwoo \delta_h\bigr\rVert_{\tau_2}^2,
\end{equation}
...
and (b) the viscous part of the adjoint test slot---exactly the Method~I
simplification of~\cite{Codina2008}.
\end{lemma}
...
\begin{itemize}[leftmargin=3.2em, itemsep=0.35ex]
  ...
  resolves it: there is $C_{\nabla\alpha} \ge 0$ with
  $h_K \norm{\nabla\alpha}_{L^\infty(K)} \le C_{\nabla\alpha}\, \alpha_K$ for all $K$ and all
  ...
  (A7), on $C_{\nabla\alpha}/C_{\mathrm{inv}}$, and on $C_2$ below (an admissible explicit
...
vertex shared by $K$ and $K' \subset S_K$. The porosity is resolved by the mesh,
$h_K\norm{\nabla\alpha}_{L^\infty(K)} \le C_{\nabla\alpha}\,\alpha_{0,K}$, so its oscillation over each
element is bounded by $\delta_\alpha \coloneqq 1 + C_{\nabla\alpha}$: $\max_{\overline{K}}\alpha \le
\delta_\alpha\,\alpha_{0,K}$ with $\alpha_{0,K}=\min_{\overline{K}}\alpha$. Since
$\alpha_{0,K}\le\alpha(\boldsymbol{x}_a)$ and $\alpha(\boldsymbol{x}_a)\le\max_{\overline{K'}}\alpha\le\delta_\alpha\,\alpha_{0,K'}$,
\begin{equation*}
...
$c_1\nu/h_K^2$, as in the proof of \cref{lem:smoothing} below. The claim follows from the explicit
...
oscillation of $\chi_1(\cdot,h)$ over an element, which vanishes as $h \to 0$ under the
smooth-grading assumption on the mesh family (bounded patch equivalence alone does not
force it; cf.~\cite[Lemma~1]{Codina2008}). All right-hand sides tend to zero
...
The stability and convergence analysis is carried out in the OSGS working norm
$\tnorm{\cdot}_{\mathrm{O}}$ of the main text; as this note treats only the OSGS method, no
ambiguity arises and we drop the subscript, writing simply $\tnorm{\cdot}$ (the ASGS norm
$\tnorm{\cdot}_{\mathrm{A}}$ appears only in the comparison lemma below). It is the
mesh-dependent norm
\begin{equation}
  \label{eq:TripleNorm}
  \tnorm{V}
...
\label{lem:definiteness}
Under \textup{(A1)--(A3)}, and with the pressure normalized to zero mean when
$\varepsilon = 0$, \cref{eq:TripleNorm} is a norm on $\Xhz$; in particular $\tnorm{V} = 0$
implies $V = 0$.
\end{lemma}

\begin{proof}
Homogeneity, symmetry and the triangle inequality are immediate; only definiteness needs an
argument, and it is here that the full Dirichlet data enter. Let $\tnorm{V} = 0$ for
$V = [\bv; q] \in \Xhz$. Then $\ViscProj\nabla\bv \equiv 0$, i.e.\ (for the deviatoric--symmetric instance kept here)
the symmetric gradient of
$\bv$ is spherical almost everywhere; since $\bv \in H^1_0(\Omega)^d$, the elementary identity
$\lVert\ViscProj\nabla\bv\rVert^2=\tfrac12\lVert\nabla\bv\rVert^2+(\tfrac12-\tfrac1d)\lVert\nabla\cdot\bv\rVert^2$
(integration by parts on $H^1_0$) gives $\norm{\nabla\bv} \le
...
zero-mean constraint when $\varepsilon = 0$) forces $q \equiv 0$. Hence $\tnorm{\cdot}$ is
...
\]
Then $\tnorm{V}_{\mathrm{A}} \le \tnorm{V}$ for all $V$.
\end{lemma}
...
\cref{lem:smoothing}. These are \emph{sufficient} conditions, and $\gamma_0$ is not claimed
...
$c_2 = 2k^2$ used in the experiments of~\cite{companion}, which are not shown to satisfy
...
(\cref{lem:smoothing}) and is not uniform in $\alpha_0$, $\nu$ or the moduli of continuity
of $\alpha$ and $\ba$.
\end{theorem}
...
\[
  \sup_{V_h\in\Xhz\setminus\{0\}} \frac{B_{\mathrm{OSGS}}(\ba; U_h, V_h)}{\tnorm{V_h}}
...
in~\cite{companion}. (iv)~The constrained pressure projection $\Pi_{2,0}$
...
\cref{eq:Method}. That solution exists and is unique: for $h \le h_0$ the inf--sup estimate
\cref{eq:InfSup} of \cref{th:stability}, on the square linear system with equal trial and
test spaces $\Xhz$, makes the system operator injective and hence invertible, so
\cref{eq:Method} is well posed. Following~\cite{companion}, the local interpolation-error quantities
are
\begin{equation}
  \label{eq:EintDef}
...
\end{equation}
and the convergence estimate is stated in the porosity-weighted broken $\ell^2$
functional
\begin{equation}
  \label{eq:ErrorFunctionL2}
  \Psi_{\mathrm O}(h)
  \coloneqq
...
inequality together with $c_1+\Dah \le c_1(1+\Dah)$, valid since $c_1 \ge 1$). We state the theorem in
...
\begin{equation}
  ...
  \;\le\; C\,\Psi_{\mathrm O}(h)\,\tnorm{V_h}.
\end{equation}
\end{lemma}

\begin{proof}
Since $U$ solves the linearized problem of \cref{eq:StrongProblem} with the stated regularity, testing the
...
the last step is \cref{eq:P5}),
which is exactly $C\,\Psi_{\mathrm O}(h)\,\tnorm{V_h}$. Each of the two broken $\ell^2$
right-hand sides is thus dominated by $\Psi_{\mathrm O}(h)$, so summing the two parts
proves \cref{eq:ConsistencyBound} in the $\ell^2$ functional (with no $\ell^2\subset\ell^1$
enlargement).
\end{proof}
...
\cref{eq:InterpolationBound} directly in the $\ell^2$ functional.
\end{proof}

\begin{lemma}[Size of the interpolation error in the triple norm]
\label{lem:interpsize}
Under \textup{(A1)--(A8)},
$\tnorm{\hat E} \le C\,\Psi_{\mathrm O}(h)$.
\end{lemma}
...
\cref{eq:ErrorFunctionL2,eq:ErrorFunction}.
\end{theorem}
...
\cref{lem:interpsize} conclude, both in the $\ell^2$ functional $\Psi_{\mathrm O}(h)$.
\end{proof}
...
C\,\sigma^{-1/2}\Psi_{\mathrm O}(h)$ by \cref{th:convergence}. In each summand of
...
\]
then \cref{eq:Tau2Expanded} gives $(\alpha_K/\nu)\,\tau_{2,K} = 1 + (c_2/c_1)\Reh$ for the
velocity term, while $\tau_{1,K} \le 1/\sigma$ from \cref{eq:P1} gives
$(\alpha_K/\nu)\,\tau_{1,K} \le \alpha_K/(\sigma\nu)$ for the pressure term. Substituting
these into $\sigma^{-1/2}\Psi_{\mathrm O}(h)$ yields \cref{eq:LtwoVelocity}. \qedhere
\end{proof}
...
be bounded head-on, and the best conversions available to the present proof---\cref{eq:P5}
and \cref{eq:P6}---cost factors $(c_1+\Dah)^{1/2}$ and $\Dah^{1/2}\le(c_1+\Dah)^{1/2}$, respectively. The Damk\"ohler factor in
\cref{eq:MainResult} is thus the factor these conversions introduce when the reactive
subscale is deleted, while the strengthened norm (in particular \cref{cor:Ltwo}) is the
...
the present, one-sided bounds. Fact (i) is \cref{cor:Ltwo}: in the regime $\Dah \gg 1$ the
...
reaction-dependent inflation of the error functional in \cref{eq:MainResult}, by
...
convention entering $\tau$), so the OSGS gradient/energy-norm bound permits a
pre-asymptotic excess that grows with $\Dah$. We do not attempt a numerical match:
...
qualitative mechanism---a reaction-dependent excess confined to the gradient/energy
...
used in~\cite[Theorem~3]{Codina2008}) are expected to give stability in the correspondingly
finer norm and a convergence estimate of the same form as \cref{eq:MainResult}. We state
this only as a prospective observation---the modified proof is not written out here---having
analyzed Method~I because it is the variant implemented in~\cite{companion} (a single
