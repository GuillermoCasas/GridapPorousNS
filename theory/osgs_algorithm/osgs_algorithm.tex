...
outputs; and a procedural pseudocode box. The presentation is
\emph{equation-first}: Section~\ref{sec:algebraic-systems} fixes the exact
algebraic system solved at each stage, and every algorithm box that
follows is read as a task against those equations.
...
of degree $(k_u, k_p) = (k, k)$ and stabilised by a Variational
...
Algorithm~\ref{alg:O} (the orchestrator, \code{solve\_system}) is \code{src/solvers/solver\_core.jl};
Algorithm~\ref{alg:B} (the Stage-I ASGS boot, \code{\_initialize\_asgs\_state!}) is
\code{src/solvers/asgs\_solver.jl}; Algorithm~\ref{alg:A} (the shared Newton kernel) is
\code{src/solvers/nonlinear.jl}; Algorithm~\ref{alg:C} (the coupled OSGS solve,
...
\paragraph{Reading order.}  The document is organised \emph{equation-first}:
§\ref{sec:algebraic-systems} pins down the exact algebraic system solved at
each stage, and the algorithm boxes that follow are read as tasks against
those equations. Those boxes are presented top-down---the orchestrator
(§\ref{sec:orchestration}), then the two stage solvers (§\ref{sec:picard},
§\ref{sec:osgs}), then the shared Newton kernel and its leaves
(§\ref{sec:newton})---each introduced by a short \emph{interface card}
summarising its callers, callees, inputs and outputs before the procedural
box. The mathematical machinery the kernel's line search needs (the
diagonally equilibrated merit function and its directional-derivative
identity) is collected in §\ref{sec:math-prelim}, immediately before that
kernel; the full parameter taxonomy (§\ref{sec:params}) and the exit-reason
map (Appendix~\ref{sec:appendix-codes}) are reference tables at the end.
...
\begin{equation}
    ...
    \mathcal{R}_{\boldsymbol{w}} U
        \;\coloneqq\; \mathcal{L}_{\boldsymbol{w}} U - F
        \;=\;
        \begin{pmatrix}
...
is the Darcy--Forchheimer drag; $\varepsilon$ is the
physical-compressibility pressure penalty $\varepsilon_{\mathrm{phys}}$
(config field \code{eps\_val}, the artificial compressibility that pins
the constant-pressure mode, $\sim 10^{-4}$ in production); and
...
are the form realised in code; the plain strong form~\eqref{eq:strong} is
their unit-porosity, penalty-free, unforced reduction. It omits the
porosity factor $\alpha$ that the porous-medium form carries on the
convective, pressure-gradient, and viscous terms and inside the divergence
($\alpha\,\nabla\!\cdot\!\boldsymbol{u} + \boldsymbol{u}\!\cdot\!\nabla\alpha$),
the penalty $\varepsilon_{\mathrm{phys}} p$, and the mass source $g$ --- all
present here. A \emph{second},
numerical penalty enters the Jacobian alone: the Codina iterative penalty
$\varepsilon_{\mathrm{num}}$ (config field \code{numerical\_epsilon},
default $0$) adds $\varepsilon_{\mathrm{num}}\,\delta p$ to the pressure
block, lags out of the residual, and so introduces no consistency error
(its effect on the tangent is noted in §\ref{sec:alg-stage1}--§\ref{sec:alg-stage2}).
...
\begin{align}
    ...
    B_S(\boldsymbol{w}; U_h, V_h)
        &\coloneqq B(\boldsymbol{w}; U_h, V_h)
...
$\partial\mathcal{L}^{*}/\partial\boldsymbol{u}$; its pressure block
multiplies $\delta p$ by $(\varepsilon_{\mathrm{phys}}+\varepsilon_{\mathrm{num}})$,
the extra $\varepsilon_{\mathrm{num}}$ (default $0$) being the root-preserving,
Jacobian-only Codina regulariser of §\ref{sec:alg-continuous}. The Picard fallback of
...
$\sigma$ is constant and \code{experimental\_reaction\_mode = "standard"},
the FE-exact terms $\sigma\boldsymbol{u}_h$ and $\varepsilon p_h$ satisfy
$(I-\Pi)(\cdot) = 0$ exactly, so they are dropped from the velocity and
pressure loads respectively; see §\ref{sec:proj-policy}.)
...
as in the Stage-I tangent of §\ref{sec:alg-stage1} (with the same
Jacobian-only $\varepsilon_{\mathrm{num}}\,\delta p$ pressure term) --- but \emph{drops} the dense coupling
...
driven by the same kernel (Algorithm~\ref{alg:A}). \emph{Sign convention.}
Throughout this document the Newton step is written in the residual-as-RHS
form $\mathcal{J}\,\delta U = \boldsymbol{b}$ with update
$U \leftarrow U - \lambda\,\delta U$ --- the convention of the code and of
every algorithm box below (equivalently the textbook
$\mathcal{J}\,\delta U = -\boldsymbol{b}$, $U \leftarrow U + \lambda\,\delta U$).
The step length
...
coupled OSGS Newton solve. The mass matrices $M_u, M_p$ are assembled
and Cholesky-factored exactly once at the start of the coupled solve. Because the
...
\code{docs/theory-code-map.md} (Section~1, the algorithm-box$\rightarrow$code map).

% ============================================================
\section{Mathematical Preliminaries}
\label{sec:math-prelim}

Two mathematical objects are consumed by the Newton kernel's line search
(§\ref{sec:newton}, immediately following) and referenced by several
algorithm boxes. We define them once here so that the boxes themselves
remain procedural.
...
safeguarded Newton routine, built on the merit function and
directional-derivative identity of the preceding section
(§\ref{sec:math-prelim}). Beneath this kernel sit three leaf
...
\For{$i = 1, \dots, N_{\mathrm{N}}$}{
    ...
    \If{$S_{\mathrm{LS}}^{\mathrm{lin}} = \mathrm{Failed}$}{
        ...
        \Return $(\boldsymbol{x},\ \mathrm{False},\ i,\ b_{\mathrm{best}},\ \mathsf{LinearSolveFailed})$\;
    }
...
\label{alg:A3}
\SetAlgoLined
\textbf{Parameters:} $\textftol$, $\textstag$, $\textxtol$, $F_{\mathrm{div}}$, $N_{\mathrm{N}}$, $N_{\mathrm{inc}}$, $N_{\mathrm{rdiv}}$.\\
...
\tcp{Residual-divergence guard (\textsc{Newton} only, active iff $N_{\mathrm{rdiv}}>0$): the Armijo search drives $\Phi$ down, so a Newton solve whose $\|\hat{\boldsymbol{b}}\|_\infty$ is \emph{climbing} escapes the merit test above; count consecutive $\ell^\infty$-residual increases beyond $F_{\mathrm{div}}$ and hand off to Picard after $N_{\mathrm{rdiv}}$.}
\If{$\mathfrak{m} = \textsc{Newton}$ \textnormal{\textbf{and}} $N_{\mathrm{rdiv}} > 0$}{
    \lIf{$\|\hat{\boldsymbol{b}}\|_\infty > F_{\mathrm{div}}\,\|\boldsymbol{b}\|_\infty$}{$I_{\mathrm{rup}} \gets I_{\mathrm{rup}} + 1$}\lElse{$I_{\mathrm{rup}} \gets 0$}
    \lIf{$I_{\mathrm{rup}} \ge N_{\mathrm{rdiv}}$}{\Return $(\mathsf{ResidualDivergence}, \mathrm{False}, \mathrm{True}, I_{\mathrm{up}})$}
}
\If{$L \le \textxtol$}{
    \If{$\|\hat{\boldsymbol{b}}\|_\infty \le \textstag$}{
        \Return $(\mathsf{NoiseFloor}, \mathrm{True}, \mathrm{True}, I_{\mathrm{up}})$\;
...
\noindent In the implementation $I_{\mathrm{up}}$ and the residual-increase
counter $I_{\mathrm{rup}}$ are carried in the \code{inc\_count} and
\code{res\_inc\_count} fields of a mutable \code{SafeIterationState} struct
that the routine updates in place; the box treats them as returned values
for mathematical clarity.

\paragraph{Why so many exit reasons?}  The arbiter distinguishes states
...
\item $\mathsf{LinesearchFailed}$, $\mathsf{MeritDivergence}$, and the
opt-in $\mathsf{ResidualDivergence}$ all trigger the Picard fallback
(Algorithm~\ref{alg:B}).
...
$\mathsf{NoProgressStall}$ is its opt-in early cousin (best residual
flat over a window), disabled in the coupled/JFNK solve.
\item $\mathsf{LinearSolveFailed}$ aborts the kernel (a linear-solver
exception, a NaN step, or a C.1 iterative non-convergence).
\end{itemize}
...
\paragraph{Production gate: the scale-free criterion supersedes the $\textftol$ exit.}
The $\mathsf{Ftol}$ test of line~5 and the $\mathsf{NoiseFloor}$ successes
(lines~10,~19,~24) are the convergence gate only when no convergence probe is
supplied---the \code{conv\_probe === nothing} fallback used by Cocquet runs and unit
tests. In a production \code{run\_simulation}, and in the parameter-sweep harness, a
\code{conv\_probe} is injected into every kernel instance, and the convergence
verdict $S_{\mathrm{conv}}$ is then the scale-free $\varepsilon_M/\varepsilon_C$
criterion of §\ref{sec:convergence-criterion}---$\varepsilon_M\le\mathrm{tol}_M$
\emph{and} $\varepsilon_C\le\mathrm{tol}_C$ on a non-degenerate iterate---evaluated
each iteration in place of line~5, with the noise-floor success branches
simultaneously disabled so that a sub-$\textstag$ but non-converged residual is no
longer silently accepted. The structural exits ($\mathsf{LinesearchFailed}$,
$\mathsf{MeritDivergence}$, $\mathsf{Xtol}$, $\mathsf{IterationCap}$) are unchanged:
they still classify \emph{failure} and drive the Picard fallback. Because the verdict
lives in this shared kernel, the same $S_{\mathrm{conv}}$ flows unchanged through the
cascade's $S_{\mathrm{N}}/S_{\mathrm{P}}$ flags (Algorithm~\ref{alg:B}) and through
the ping-pong segments of the coupled solve (§\ref{sec:osgs}): a Picard pass that
reaches $\varepsilon_M\le\mathrm{tol}_M$ is recognised as a genuine success rather
than re-judged against a residual target re-anchored per segment (the failure mode
the 2026-06 cleanup removed).
...
Cholesky-factored \emph{once} at the top of the coupled solve (Algorithm~\ref{alg:C}), and every
...
entirely. By default Algorithm~\ref{alg:C} issues a \emph{single}
safeguarded Newton solve (Algorithm~\ref{alg:A}) on the primary unknown
$U_h$, whose residual re-slaves $\boldsymbol{\pi}_h$ on every evaluation.
(Two opt-in coupled-solve variants exist, both off by default and mutually
exclusive: \code{pingpong\_enabled} runs a Newton$\leftrightarrow$Picard
ping-pong of such inner solves, and \code{osgs\_anderson\_enabled} runs an
Anderson-accelerated staggered iteration; the JFNK path of §\ref{sec:jfnk}
is a third, also mutually exclusive.) Convergence is the scale-free
$\varepsilon_M/\varepsilon_C$ gate of the inner Newton
(§\ref{sec:convergence-criterion}), identical across any ping-pong
segments; there is no separate OSGS stopping criterion, no drift metric,
and no warmup schedule.
...
dropped action without forming it.) The frozen-$\boldsymbol{\pi}_h$ tangent
is assembled with the \emph{live} projection $\boldsymbol{\pi}_h(U_h)$---the
same value the residual uses at that iterate---not a zero placeholder. The
projection value \emph{does} enter the Exact-Newton product-rule terms
$\mathrm{d}\tau_1\cdot(\mathcal{R}-\boldsymbol{\pi}_h)$ and
$\mathrm{d}\mathcal{L}^{*}\cdot(\mathcal{R}-\boldsymbol{\pi}_h)$; passing a
literal zero there silently replaces $(\mathcal{R}-\boldsymbol{\pi}_h)$ by
$\mathcal{R}$, an inexactness \emph{beyond} the intended
$\partial\boldsymbol{\pi}_h/\partial U_h$ drop. With the live
$\boldsymbol{\pi}_h$ the assembled matrix is the \emph{exact}
frozen-$\boldsymbol{\pi}_h$ tangent---it reproduces a finite-difference of the
frozen-$\boldsymbol{\pi}_h$ residual to machine precision, whereas the
zero-placeholder tangent carries an $O(\boldsymbol{\pi}_h)$ relative error
($\sim\!3\%$ on a convective test)---so the only remaining approximation is
the deliberate, sparsity-preserving $\partial\boldsymbol{\pi}_h/\partial U_h$
drop. Because $\mathcal{R}-\boldsymbol{\pi}_h\to 0$ at the fixed point, the
$\mathrm{d}\tau\,/\,\mathrm{d}\mathcal{L}^{*}$ terms vanish there: the
converged solution is unchanged and only the Newton path is refined. The
projection is computed once per iterate and shared by the residual and the
Jacobian (a DOF-keyed cache), so the exact tangent costs no extra projection.
...
$\boldsymbol{\pi}_h \equiv 0$. Hence $\boldsymbol{\pi}_h^0 = 0$ is the
projection value \emph{in force during the ASGS solve}---a bootstrap
convention, not a claim that the slaved map vanishes at $U_h^{\mathrm{ASGS}}$:
in general $\Pi(\mathcal{R}_{\boldsymbol{u}}U_h^{\mathrm{ASGS}}) \neq 0$,
because the ASGS root satisfies the \emph{stabilised} weak form (with
$\boldsymbol{\pi}_h$ modelled as $0$), not the projection equation. No
separate pre-seeding of $\boldsymbol{\pi}_h^0$ is performed, and none is
needed: the first OSGS residual evaluation re-projects
$\boldsymbol{\pi}_h^1 = \Pi(\mathcal{R}_{\boldsymbol{u}}U_h^{\mathrm{ASGS}})$
naturally, the first non-trivial projection of the run.
...
\moduleinterface
    ...
    {Stage~I state $U_h^{\mathrm{ASGS}}$; residual operators $(\mathcal{R}_u, \mathcal{R}_p)$ and projection spaces $(V_{\mathrm{free}}, Q_{\mathrm{free}})$, from which it assembles and Cholesky-factors the mass matrices $(M_u, M_p)$ once}
...
\KwIn{$U_h^{\mathrm{ASGS}}$; residual operators $(\mathcal{R}_u, \mathcal{R}_p)$; projection spaces $(V_{\mathrm{free}}, Q_{\mathrm{free}})$.}
\KwOut{$(S, \boldsymbol{\pi}_{h,u}, \pi_{h,p})$.}
\vspace{0.2cm}\hrule\vspace{0.2cm}
assemble $M_u, M_p$ on $(V_{\mathrm{free}}, Q_{\mathrm{free}})$ and Cholesky-factor once \tcp*{SPD, state-independent; reused by every re-projection}
...
\Indm
\vspace{0.1cm}
$(S^{\mathrm{inner}}, U_h^\star, \cdot) \gets \mathrm{ExactNewtonPipeline}(U_h^{0},\ \mathrm{op})$ \tcp*{Alg.~\ref{alg:A}; budget $N_{\mathrm{OSGS}}$; on failure keep best-seen iterate, restore $U_h^{\mathrm{ASGS}}$ only if non-finite}
...
\extline{\If{$\code{mms\_cfg.enabled}$}{
    ...
    \If{$E_{u,2} > \kappa_{\mathrm{rate}}\, h^{\,k_u+1}$}{ record $\code{coupled\_at\_suboptimal\_rate}$ \tcp*{post-solve check only; no extra cycles} }
}}
...
once at the converged iterate for diagnostics. In the default mode there is no outer loop:
the only iteration is the inner Newton's, and its scale-free
$\varepsilon_M/\varepsilon_C$ gate (§\ref{sec:convergence-criterion}) is the sole convergence test. The inner-success policy
...
$\mathsf{MeritDivergence}$ (\code{merit\_divergence\_escaped}),
$\mathsf{ResidualDivergence}$ (\code{residual\_divergence\_escaped}), or
$\mathsf{LinearSolveFailed}$ (\code{linear\_solve\_failed}) (see Algorithm~\ref{alg:A3});
...
factorisations are assembled once at the top of Algorithm~\ref{alg:C} and reused:
...
(\code{reaction\_model = "Constant\_Sigma"} in \code{physical\_properties})
\emph{and} \code{sol.experimental\_reaction\_mode = "standard"}, the solver
selects the \code{ProjectResidualWithoutReactionWhenConstantSigma} policy
(\code{run\_simulation.jl}), which omits the
reaction term $\sigma \boldsymbol{u}_h$ from the residual
...
no cost. The same policy applies the symmetric trim on the pressure side:
the linear penalty $\varepsilon\,p_h$ is likewise FE-exact for constant
$\varepsilon$, so $(I-\Pi)(\varepsilon p_h) = 0$ and it too is dropped from
the pressure residual before projection (\code{projection.jl}). This
corresponds to Section~4.4 of the parent paper. Note that
...
\paragraph{Status.}  \textbf{Implemented} (2026-06-26), opt-in behind the
config flag \code{osgs\_jfnk\_enabled} (OFF by default; when off the coupled
solve is the frozen-$\boldsymbol{\pi}_h$ inexact Newton of
Algorithm~\ref{alg:C}, bit-identical). It is \code{[code-actual]}:
\code{JFNKLinearSolver} (src/solvers/linear\_solvers.jl) is a drop-in
matrix-free \code{LinearSolver} plugged into the existing
\code{SafeNewtonSolver} by \code{\_osgs\_jfnk\_solve!}
(src/solvers/osgs\_solver.jl) --- realising literally the ``change exactly one
thing: the inner linear solve'' of §\ref{sec:jfnk-change}, so the outer Newton,
line search, safeguards, and the [C.1] honesty contract are inherited
unchanged. A non-converged inner GMRES is surfaced as a typed
\code{GMRESNotConvergedError} and rolled back; on a structural inner
failure, \code{\_osgs\_jfnk\_solve!} re-runs the frozen-$\boldsymbol{\pi}_h$
direct coupled solve of Algorithm~\ref{alg:C} from the best iterate, so the
JFNK path is never worse than the direct one. The enabling Phase-0 measurement (the frozen-$\boldsymbol{\pi}_h$
preconditioner is viable: $1$--$21$ inner Krylov vectors; $N_j\,{=}\,2$--$3$
vs $N_c\,{=}\,60$) is in \code{docs/solver/jfnk-phase0-preconditioner-gate.md};
the regression anchors are \code{test/quick/osgs\_jfnk\_quick\_test.jl} (the
matrix-free action equals the true full tangent; the production solve converges
to the same root as a full-Jacobian Newton; [C.1] starved-GMRES honesty) and
\code{test/extended/jfnk\_equivalence\_extended\_test.jl} (same MMS root,
JFNK iterations $\le$ frozen-$\boldsymbol{\pi}_h$, ASGS byte-identical). This
section is the method description; the paragraphs below explain what the inner
solve recovers and what changes (and what does not) when it is switched on.
...
\code{src/solvers/osgs\_solver.jl}).
...
\begin{equation}
    ...
    \mathcal{J}_{\mathrm{full}}(U_h)
        = \underbrace{\frac{\partial \boldsymbol{B}_S}{\partial U_h}
            ...
            \displaystyle \mathcal{J}_{\bar\pi}(U_h)\ \mathrm{(frozen-}\boldsymbol{\pi}_h\mathrm{)}}
        \;-\;
        \underbrace{\frac{\partial \boldsymbol{L}_S}{\partial \boldsymbol{\pi}_h}
            \,\frac{\partial \boldsymbol{\pi}_h}{\partial U_h}}_{
            \displaystyle \mathcal{C}(U_h)\ \mathrm{(coupling)}},
    \qquad
...
\delta U_h = \boldsymbol{F}$ (the residual-as-RHS box convention of
§\ref{sec:newton}, with update $U_h \leftarrow U_h - \lambda\,\delta U_h$)
is therefore an \emph{inexact} Newton step, and the coupled iteration
converges only linearly (super-linearly at best), with the contraction
rate set by the spectral radius of $\mathcal{J}_{\bar\pi}^{-1}\mathcal{C}$.
...
Algorithm~\ref{alg:A1} (residual-as-RHS, as in that box) is replaced by a
matrix-free GMRES whose matrix--vector product is the directional finite
difference of the \emph{full} coupled residual,
\begin{equation}
...
\end{equation}
Here $b_{\mathrm{fd}}$ is the Brown--Saad finite-difference base --- the
config field \code{sol.osgs\_jfnk\_fd\_epsilon} (default
$10^{-8}\approx\sqrt{\varepsilon_{\mathrm{mach}}}$, validated $>0$) --- and
$U_h$ is the current Newton iterate, recorded by the Jacobian closure as
the finite-difference base point (distinct from the line-search trial
points at which $\boldsymbol{F}$ is also evaluated).
...
The frozen-$\boldsymbol{\pi}_h$ Jacobian~\eqref{eq:jfnk-frozen-jac} ---
re-assembled and re-factored at the current iterate $U_h$, exactly as in
the direct coupled step --- is reused as the GMRES \emph{preconditioner}
across that step's Krylov vectors (no extra factorisation per matvec).
Because it is $\mathcal{J}_{\mathrm{full}}$ minus only the compact coupling
term, the \emph{left}-preconditioned operator
$\mathcal{J}_{\bar\pi}^{-1}\mathcal{J}_{\mathrm{full}}
 = I - \mathcal{J}_{\bar\pi}^{-1}\mathcal{C}$
...
Left-preconditioned GMRES (the \code{Pl} argument to \code{gmres!},
\code{linear\_solvers.jl}) then converges in a handful of Krylov vectors,
each preconditioner application being a single solve against the existing
factorisation --- a back/forward substitution under the default direct
\code{LU} backend, or an inner ILU--GMRES sweep under the low-memory
\code{ILU\_GMRES} backend.

\moduleinterface
    {Coupled Newton step (the JFNK variant of Algorithm~\ref{alg:A})}
    {Coupled residual $\boldsymbol{F}$ of~\eqref{eq:jfnk-coupled-residual} (re-projects $\boldsymbol{\pi}_h$); factored $\mathcal{J}_{\bar\pi}$ as left preconditioner}
    {Current iterate $U_h$; residual $\boldsymbol{F}(U_h)$; assembled+factored $\mathcal{J}_{\bar\pi}(U_h)$; forcing tolerance $\eta$, Krylov cap $N_{\mathrm{K}}$, restart $\ell$}
    {Newton step $\delta U_h$ (full-tangent, matrix-free)}

\begin{algorithm}[H]
\caption{$\mathrm{JFNKStep}$ \emph{(implemented; opt-in \code{osgs\_jfnk\_enabled})}: full-tangent Newton step via preconditioned matrix-free GMRES.}
\label{alg:jfnk}
\SetAlgoLined
\textbf{Parameters:} fixed forcing tolerance $\eta = \code{sol.osgs\_jfnk\_gmres\_rel\_tol}$ (default $10^{-2}$; no Eisenstat--Walker adaptivity is implemented), Krylov cap $N_{\mathrm{K}} = \code{sol.osgs\_jfnk\_gmres\_maxiter}$, restart $\ell = \code{sol.osgs\_jfnk\_gmres\_restart}$.\\
...
define left preconditioner apply $\mathcal{P}^{-1}[\boldsymbol{r}] \gets \mathcal{J}_{\bar\pi}^{-1}\boldsymbol{r}$ \tcp*{one solve against the existing frozen-$\boldsymbol{\pi}_h$ factorisation}
$\delta U_h \gets \mathrm{GMRES}\bigl(\mathcal{A},\, \boldsymbol{F}(U_h);\ \text{left precond.}\ \mathcal{P}^{-1},\ \text{tol}\ \eta,\ \text{cap}\ N_{\mathrm{K}},\ \text{restart}\ \ell\bigr)$ \tcp*{step then enters Alg.~\ref{alg:A2}: $U_h \leftarrow U_h - \alpha\,\delta U_h$}\;
\Return $\delta U_h$\;
\end{algorithm}
...
$\eta > 0$) Krylov step satisfies it only up to the Krylov residual, so in
principle the slope test relaxes to the inexact-Newton form
$D \le -2\Phi(1-\eta)$. The implemented JFNK \emph{deliberately reuses the
existing exact-slope line search} ($D = -2\Phi$, Algorithm~\ref{alg:A2})
unchanged: at the production forcing tolerance $\eta \approx 10^{-2}$ the
$(1-\eta)$ factor is a $1\%$ effect, and the stricter exact slope is
conservative-but-safe --- it can only backtrack marginally more, never accept a
non-descent step --- which keeps the safeguard stack and its behavior-preservation
guarantee verbatim. (Should a much larger $\eta$ ever be configured, the relaxed
slope would be the principled change.)
...
Cholesky-cached projection) and one preconditioner solve (one solve
against the cached frozen-$\boldsymbol{\pi}_h$ factorisation). No new
factorisation, no dense storage.
...
\paragraph{Phase-0 measurement (2026-06-26): the preconditioner is
viable.} A clean re-measurement on equal-order P1/P1 cells (mild
$\nu{=}1,\sigma{=}1$ and stiff/convective $\nu{=}0.01,\sigma{=}100$) confirms
the accounting above. The full-tangent Newton converges in $N_j = 2$--$3$
steps (quadratic) where the frozen-$\boldsymbol{\pi}_h$ inexact Newton fails
to converge in $60$ (and \emph{diverges} on the stiff cells), and the dropped
$\mathcal{C}$ is large ($\|\mathcal{C}\|/\|\mathcal{J}_{\mathrm{full}}\| =
0.15$ mild, $0.90$--$0.97$ stiff), so it---not slow setup---is the rate
limiter. The decisive metric: the factored frozen-$\boldsymbol{\pi}_h$
Jacobian preconditions the matrix-free full-J GMRES to $1$--$4$ Krylov vectors
(mild) and a $\le 16$--$21$ peak (stiff/convective) at an inexact-Newton inner
tolerance, with the directional-FD matvec~\eqref{eq:jfnk-matvec} reproducing the
counts exactly. Two refinements to the spectral argument
of~\S\ref{sec:jfnk-change}: (i) the sufficient condition
$\|\mathcal{J}_{\bar\pi}^{-1}\mathcal{C}\| < 1$ holds for the mild cells
($\approx 0.88$) but is \emph{violated} for the stiff cells
($\approx 30$--$67$); GMRES still converges in a handful of vectors because the
super-unit eigenvalues are few and clustered, i.e.\ the Krylov polynomial beats
the worst-case spectral-norm bound. (ii) The preconditioner must use the
\emph{residual-consistent} tangent: adding the Codina penalty
$\varepsilon_{\mathrm{num}}$ (Jacobian-only, root-preserving) to the
preconditioner pulls it \emph{off} $\mathcal{J}_{\mathrm{full}}$ and
\emph{degrades} the Krylov count, so JFNK preconditions with
$\varepsilon_{\mathrm{num}}{=}0$ and relies on the physical
$\varepsilon_{\mathrm{phys}}$ for the pressure-mode conditioning. Full data:
\code{docs/solver/jfnk-phase0-preconditioner-gate.md}.
...
answers \emph{whether} an iterate is converged, not \emph{how} to step. The full design rationale lives
in \code{docs/theory-code-map.md} (Section~3) and the standalone note \code{theory/scale\_free\_gate\_note/}.
...
special-casing by polynomial order and is valid for the equal-order $P_{k_u}/P_{k_p}$ pair. Two
properties follow: (i) $\varepsilon_M\to0$ at the discrete solution---it reports genuine convergence
rather than plateauing at the $O(h^{k_u})$ floor of the \emph{strong} residual; and (ii) the
...
conversion factor \emph{measured from the iterate}: writing $\Lambda/\alpha_\infty$ for the
viscous+inertial+Darcy envelope, steady-state balance gives $\|p^k\|\sim(\Lambda/\alpha_\infty)\,
\|\nabla(\alpha\boldsymbol{u}^k)\|$, and substituting the measured
$\Lambda/\alpha_\infty=\|p^k\|/\|\nabla(\alpha\boldsymbol{u}^k)\|$ into
...
This criterion is the authoritative convergence verdict in both production and the parameter-sweep
harness: \code{solve\_system} injects it (as the \code{conv\_probe}) into every kernel instance
(§\ref{sec:newton}). The harness additionally supplies the discretisation-coupled scalar
tolerance~\eqref{eq:dynamic-ftol} as the \emph{fallback} floor $\textftol$, keyed to $h$ so the linear
solve is not over-solved relative to the $O(h^{k_u+1})$ discretisation error; but the stopping decision
itself is this section's scale-free $\varepsilon_M/\varepsilon_C$ gate, not a residual target.
The earlier per-field intrinsic-scale rule of Sec.~\ref{sec:harness-perfield}---which re-anchored the
target to the per-segment initial residual---was removed in the 2026-06 cleanup (see the superseded
note there); this criterion requires no manufactured solution, no $h$, and no initial-residual anchor,
basing the normalisation entirely on the iterate's own force balance and flux structure.
...
of its own: it terminates on the scale-free $\varepsilon_M/\varepsilon_C$
gate (§\ref{sec:convergence-criterion}) of its single inner Newton (Algorithm~\ref{alg:A3}). There is no
...
(a flat \code{SolverConfig} \code{@kwdef} struct). The
...
$\textftol$              & \code{sol.ftol}                          & Absolute $\ell^\infty$ residual floor; the \code{conv\_probe===nothing} fallback gate (Cocquet/unit tests). Superseded in production by the scale-free criterion below. \\
$\mathrm{tol}_M$         & \code{sol.eps\_tol\_momentum}            & Scale-free momentum tolerance: converged when $\varepsilon_M\le\mathrm{tol}_M$ (§\ref{sec:convergence-criterion}). \\
$\mathrm{tol}_C$         & \code{sol.eps\_tol\_mass}                & Scale-free mass tolerance: converged when $\varepsilon_C\le\mathrm{tol}_C$. \\
...
$N_{\mathrm{rdiv}}$      & \code{sol.newton\_residual\_divergence\_patience} & Consecutive $\ell^\infty$-residual increases (beyond $F_{\mathrm{div}}$) tolerated in \textsc{Newton} mode before handing off to Picard; $0$ disables the guard (default). \\
$N_{\mathrm{stall}}$     & \code{sol.newton\_stall\_window}         & Window for the no-progress stall guard; $0$ disables it (overridden to $0$ in the coupled/JFNK solve). \\
$\delta_{\mathrm{stall}}$ & \code{sol.newton\_stall\_min\_rel\_improvement} & Relative $\|\boldsymbol{b}\|_\infty$ drop counting as progress within that window. \\
\midrule
\multicolumn{3}{l}{\emph{OSGS coupled solve}} \\
$N_{\mathrm{OSGS}}$      & \code{sol.newton\_iterations} (shared)   & The coupled OSGS solve has no dedicated iteration knob: it reuses the shared inner-Newton budget $N_{\mathrm{N}}$ (only \code{stall\_window} is overridden to $0$). The projection $\boldsymbol{\pi}_h$ is re-slaved every residual evaluation, so there is no outer-loop budget or drift criterion. \\
\midrule
\multicolumn{3}{l}{\emph{OSGS JFNK inner solve} (opt-in, \code{sol.osgs\_jfnk\_enabled}; §\ref{sec:jfnk})} \\
\code{jfnk}              & \code{sol.osgs\_jfnk\_enabled}           & Boolean: replace the frozen-$\boldsymbol{\pi}_h$ direct inner solve with the matrix-free full-tangent GMRES of §\ref{sec:jfnk}. Default \code{false}; mutually exclusive with \code{osgs\_anderson\_enabled}. \\
$\eta$                   & \code{sol.osgs\_jfnk\_gmres\_rel\_tol}   & Fixed inner-GMRES forcing tolerance (no Eisenstat--Walker); default $10^{-2}$, validated $\in(0,1)$. \\
$N_{\mathrm{K}}$         & \code{sol.osgs\_jfnk\_gmres\_maxiter}    & Inner-GMRES iteration cap per Newton step; default $100$, $\ge 1$. \\
$\ell$                   & \code{sol.osgs\_jfnk\_gmres\_restart}    & GMRES restart length; default $50$, $\ge 1$. \\
$b_{\mathrm{fd}}$        & \code{sol.osgs\_jfnk\_fd\_epsilon}       & Brown--Saad finite-difference base in $\varepsilon = b_{\mathrm{fd}}(1+\|U_h\|)/\|\boldsymbol{v}\|$; default $10^{-8}$, $>0$. \\
\bottomrule
\end{tabularx}
\end{table}
...
$\mathrm{CoupledOSGSSolve}$ (Alg.~\ref{alg:C})  & \code{solve\_osgs\_stage!} (\code{src/solvers/osgs\_solver.jl}), dispatched from \code{solve\_system}: one \code{safe\_fe\_solve!} call on Algorithm~\ref{alg:A} whose residual re-slaves $\boldsymbol{\pi}_h$ at every evaluation. \\
...
$\mathrm{JFNKStep}$ (Alg.~\ref{alg:jfnk})               & \code{\_osgs\_jfnk\_solve!} (\code{osgs\_solver.jl}) wiring \code{JFNKLinearSolver} (\code{linear\_solvers.jl}) as a drop-in \code{LinearSolver} into the shared \code{SafeNewtonSolver}; active iff \code{sol.osgs\_jfnk\_enabled}. \\
\bottomrule
\end{tabularx}
\end{table}
...
$\mathsf{MeritDivergence}$    & \code{"merit\_divergence\_escaped"}     & Merit $\Phi$ expanded by $\ge F_{\mathrm{div}}$ for $N_{\mathrm{inc}}$ consecutive steps (\textsc{Newton}); $\|\boldsymbol{b}\|_\infty$ likewise (\textsc{Picard}). \\
$\mathsf{ResidualDivergence}$ & \code{"residual\_divergence\_escaped"}   & \textsc{Newton}, opt-in ($N_{\mathrm{rdiv}}>0$): $\|\boldsymbol{b}\|_\infty$ rose by $\ge F_{\mathrm{div}}$ for $N_{\mathrm{rdiv}}$ consecutive steps; hands off to Picard. \\
...
$\mathsf{NoProgressStall}$    & \code{"no\_progress\_stall"}            & Opt-in ($N_{\mathrm{stall}}>0$): best $\|\boldsymbol{b}\|_\infty$ failed to improve by the relative margin over the stall window; bails for caller fallback. Disabled in the coupled/JFNK solve ($N_{\mathrm{stall}}=0$). \\
$\mathsf{LinearSolveFailed}$     & \code{"linear\_solve\_failed"}             & Linear-solver exception, NaN step, or controlled iterative non-convergence (C.1: \code{GMRESNotConvergedError} / \code{ILUFactorizationFailure}). \\
...
if the velocity $L^2$ error exceeds the expected $O(h^{k_u+1})$ rate,
...
converged FE error against the $O(h^{k_u+1})$ rate but does not gate
...
root and the true PDE solution, which behaves as $O(h^{k_u+1})$.
...
\begin{equation}
    ...
    = \max\!\Bigl(\textftol^{\,\mathrm{static}},\
        \min(C_{\mathrm{ceil}},\ C_{\mathrm{sf}} \cdot h^{k_u+1}) \Bigr),}
\end{equation}
...
\begin{tcolorbox}[colback=red!4!white, colframe=red!55!black, boxrule=0.5pt, arc=2pt, left=6pt, right=6pt, top=4pt, bottom=4pt]
\small
\textbf{Superseded (2026-06).} The dimensionless-rate-per-field gate described in
this subsection---the per-block scale proxy~\eqref{eq:per-field-scale-resolve}, the
Bernoulli surrogate, the rate $\varepsilon_k=C_{\mathrm{sf}}h^{k_u+1}$
of~\eqref{eq:per-field-rate}, and the block-wise
predicates~\eqref{eq:per-field-convergence}--\eqref{eq:per-field-honest}---was the
\emph{re-anchored relative} convergence machinery (\code{relative\_ftol\_per\_field},
\code{relative\_stagnation\_noise\_floor\_per\_field}). It was \emph{removed} from
\code{SafeNewtonSolver} in the 2026-06 cleanup: re-anchoring the target to the
per-segment initial residual created moving goalposts across
Newton$\leftrightarrow$Picard handoffs. In the current code the harness passes the
scalar discretisation-coupled $\textftol^{\mathrm{dyn}}$ of~\eqref{eq:dynamic-ftol}
as the fallback floor, and the \emph{authoritative} convergence verdict---in the
harness exactly as in production---is the scale-free $\varepsilon_M/\varepsilon_C$
criterion of §\ref{sec:convergence-criterion}. The per-field residual \emph{vectors}
survive only as an assembly detail (\code{effective\_ftol\_per\_field}, now filled
uniformly from the scalar \code{ftol}); the rate/proxy construction below does not.
It is retained here as a record of what the gate used to be.
\end{tcolorbox}
...
\begin{equation}
    ...
    \textcolor{extcolor}{
    R_{k}(\boldsymbol{b}) \coloneqq \|\boldsymbol{b}[\mathcal{B}_{k}]\|_{\infty},
    \qquad
    \mathcal{U}_{k}^{\mathrm{raw}}(\boldsymbol{x}) \coloneqq \|\boldsymbol{x}[\mathcal{B}_{k}]\|_{\infty},
    \qquad k \in \{u, p\}.}
\end{equation}
...
\begin{equation}
    ...
    \textcolor{extcolor}{
    \mathcal{U}_{k}(\boldsymbol{x}) \coloneqq
    \begin{cases}
...
\begin{equation}
    ...
    \coloneqq \max\!\Bigl(\textftol^{\,\mathrm{static}},\
...
\begin{equation}
    ...
    \textcolor{extcolor}{
    \varepsilon_{u} = \varepsilon_{p} = C_{\mathrm{sf}} \cdot h^{k_u+1},}
\end{equation}
...
expected $O(h^{k_u+1})$ FE discretisation error---independently of
...
\begin{equation}
    ...
    \rho_{\mathrm{N}}\!\bigl(\hat{\boldsymbol{b}}, \boldsymbol{x}^{(i)}\bigr)
        ...
        \qquad \tau^{\mathrm{N}}_{k} \coloneqq \max\!\Bigl(\textstag^{\,\mathrm{dyn}},
...
\begin{table}[h]
\centering
\caption{\extline{Numerical parameters of the parameter-sweep harness adaptations (extension-only). The \code{dynamic\_*} fields are harness-frame quantities defined in \code{test/extended/harness\_dynamic\_budget.jl} (relocated out of \code{SolverConfig} on 2026-06-25); \code{condition\_noise\_floor\_*} remains a \code{sol} field.}}
...
\textbf{Symbol} & \textbf{Field in code} & \textbf{Role / effect} \\
\midrule
$C_{\mathrm{ceil}}$, $C_{\mathrm{sf}}$ & \code{dynamic\_ftol\_ceiling}, \code{dynamic\_ftol\_spatial\_safety\_factor} & Clamps for the discretisation-coupled residual tolerance \eqref{eq:dynamic-ftol}, \S\ref{sec:harness-ftol}; $C_{\mathrm{sf}}$ also sets the per-field dimensionless rate \eqref{eq:per-field-rate}. \\
$\varepsilon_{k}$, $\varepsilon^{\mathrm{N}}_{k}$ & (built from $C_{\mathrm{sf}}$, $n^{\mathrm{sf}}$, $h$, $k_u$) & Dimensionless per-field convergence and noise-floor rates supplied to the inner Newton kernel as \code{relative\_ftol\_per\_field} and \code{relative\_stagnation\_noise\_floor\_per\_field}, \S\ref{sec:harness-perfield}. \\
...
$\mathrm{Re}^{\mathrm{th}}, N_{\mathrm{P}}^{\mathrm{Re}}$ & \code{dynamic\_picard\_re\_*} & Per-case Picard widening trigger and budget for high Reynolds. \\
$\mathrm{Da}^{\mathrm{th}}, N_{\mathrm{P}}^{\mathrm{Da}}$ & \code{dynamic\_picard\_da\_*} & Per-case Picard widening trigger and budget for high Damk\"ohler. \\
$\mathrm{Re}^{\mathrm{th,N}}, N_{\mathrm{N}}^{\mathrm{Re}}$ & \code{dynamic\_newton\_re\_*} & Per-case Newton widening trigger and budget for high Reynolds. Mirrors the Picard pair; landed 2026-05-19 (\code{fa8aaec}) to absorb the transient smoothing phase the corrected mode-aware divergence safeguard exposes when Newton has to backtrack through residual-landscape spikes at $\mathrm{Re} \ge \mathrm{Re}^{\mathrm{th,N}}$. \\
\bottomrule
\end{tabularx}}
\end{table}
