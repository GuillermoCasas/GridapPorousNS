...
\subsection{Mass: flux-divergence over flux-gradient}
\begin{equation}
    \label{eq:cc-epsC}
    \varepsilon_C = \frac{\bigl\|\,\varepsilon\,p^k + \nabla\!\cdot\!(\alpha\boldsymbol{u}^k)\,\bigr\|}
                         {\|\nabla(\alpha\boldsymbol{u}^k)\|_F},
    \qquad
    \nabla(\alpha\boldsymbol{u}^k)=\alpha\nabla\boldsymbol{u}^k
        + \boldsymbol{u}^k\!\otimes\!\nabla\alpha ,
\end{equation}
the dilatation of the porous flux $\boldsymbol{q}=\alpha\boldsymbol{u}$ normalised by the magnitude of
the flux variation (Frobenius norm of the full flux gradient, which retains the porosity-gradient
structure $\boldsymbol{u}\!\otimes\!\nabla\alpha$). Both norms are global $L^2(\Omega)$ and built only
from $\boldsymbol{u}^k$ and the known field $\alpha$, so the ratio is dimensionless by construction.
The penalty term $\varepsilon p^k$ is retained for strictness (negligible for the production
$\varepsilon\sim10^{-4}$, but it keeps early iterations from looking over-converged). For constant
$\alpha$ and $\varepsilon=0$ it reduces to the textbook
$\|\nabla\!\cdot\!\boldsymbol{u}\|/\|\nabla\boldsymbol{u}\|$.

\paragraph{The $\sqrt{d}$ bound (and self-check).} Because
$\nabla\!\cdot\!\boldsymbol{q}=\operatorname{tr}(\nabla\boldsymbol{q})$ and, by Cauchy--Schwarz,
$(\operatorname{tr}A)^2=(A:I)^2\le(A:A)(I:I)=d\,(A:A)$ pointwise, integration gives
$\|\nabla\!\cdot\!\boldsymbol{q}\|^2\le d\,\|\nabla\boldsymbol{q}\|_F^2$, hence
$\varepsilon_C\le\sqrt{d}$ in exact arithmetic (modulo the small $\varepsilon p$ term). This ceiling
is a cheap correctness check: on a developed iterate a value above $\sqrt{d}$ flags a
quadrature/assembly bug. It is \emph{logged as a warning, not asserted}---in the degenerate roundoff
regime the signed divergence sum can suffer catastrophic cancellation and nudge the computed ratio
slightly past $\sqrt{d}$ without any real defect.
...
identically and leaves~\eqref{eq:cc-epsC}. The conversion factor carries units---that is what a
