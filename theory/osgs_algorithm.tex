...
must enter the exact-Newton quadratic basin before the OSGS staggered
loop can fragment the state via projections, and an extra Picard pass
...
Stage~II inner cascade (called from Algorithm~\ref{alg:C}) accepts any
non-structural Newton exit (including $\mathsf{stagnation\_noise\_floor\_reached}$,
$\mathsf{xtol\_stagnation}$, $\mathsf{max\_iters\_stagnation}$): the
OSGS outer loop re-evaluates convergence after every staggered
iteration, so a non-tight inner exit is just outer-loop progress
rather than a structural failure. The asymmetry exists in code
because of this difference in what "good enough" means at each site;
the Algorithm~\ref{alg:B} pseudocode above is the common subset.
...
\item \textbf{Stage I (ASGS init)} treats it as a structural failure
  ...
  Picard.
\item \textbf{Stage II (OSGS inner cascade)} treats it as a
  single-iteration partial success: $\mathrm{iter\_count}$ is
  incremented but no fallback fires. The outer staggered loop will
  re-evaluate convergence on the next iteration.
...
\paragraph{Implementation note (2026-06): explicit cascade policy and the opt-in ping-pong.}
...
Stage-I~Newton-1 $=(\mathrm{false},\mathrm{false},\mathrm{true})$, Stage-I~Newton-2
$=(\mathrm{true},\mathrm{false},\mathrm{true})$ (it accepts a noise-floor finish though
Newton-1 does not, but still rejects a soft stall), and OSGS-inner
$=(\mathrm{true},\mathrm{true},\mathrm{false})$. Two \emph{opt-in, default-off} efficiency
variants reuse this machinery without altering the canonical algorithm above:
(i) an adaptive Newton$\leftrightarrow$Picard \emph{ping-pong} that replaces the one-way
Newton$\to$Picard$\to$Newton cascade in both instantiations when
$\mathsf{pingpong\_enabled}$, running Picard only until $\|\mathcal{R}\|_\infty$ has dropped
$\mathsf{pingpong\_picard\_gain\_orders}$ orders (just enough to re-enter Newton's basin)
before returning to Newton, bounded by $\mathsf{pingpong\_max\_swaps}$; and (ii) an OSGS
plateau short-circuit (Alg.~\ref{alg:C}) that accepts a single passing MMS-plateau check once
the state drift has reached the \emph{tight} machine floor $x_{\mathrm{diff}}\le\mathsf{osgs\_tol}$
in \code{state\_drift} mode, when $\mathsf{osgs\_plateau\_machine\_floor\_shortcut}$ is set.
Both default off (canonical behaviour unchanged); see
\code{docs/solver/efficiency-ideas.md} and \code{docs/solver/algorithm-code-mapping.md}.
...
Algorithm~\ref{alg:D} is mode-agnostic; it compares the last two
entries of the running error history $\mathcal{H}$ regardless of how
those entries were produced. However, the semantic content of one
$\mathcal{H}$-entry differs between the two paths:
in the ASGS extension loop, one entry corresponds to one extra
\emph{single-Newton-step} on top of the already-converged
$U_h^{\mathrm{ASGS}}$, whereas in the OSGS path one entry
corresponds to one full \emph{outer staggered iteration} (inner
cascade + residual reprojection + Anderson mixing). The plateau
tolerance $\tau_{\mathrm{err}}$ is calibrated against the relative
change between consecutive entries; users tuning $\tau_{\mathrm{err}}$
should keep in mind that the per-entry cost---and therefore the
expected per-entry relative-change scale---is different in the two
paths. The current default is conservative enough that either path
satisfies it.}

\extline{\paragraph{Budget exhaustion is not a verification failure
of the solver.} When the OSGS branch reaches its inflated outer-loop
budget $N_{\mathrm{OSGS}}^{\mathrm{eff}}$ without the plateau test
firing, the solver returns
$(S_{\mathrm{solver}}, S_{\mathrm{plat}}) = (\mathrm{True}, \mathrm{False})$
with \code{diag\_cache["mms\_stop\_reason"] = "mms\_budget\_exhausted"}.
The base OSGS fixed point converged (this is what $S_{\mathrm{conv}}$
firing at least once established); the plateau verifier ran out of
samples. Callers that care about FE-error verification must read
$S_{\mathrm{plat}}$ explicitly: a $\mathrm{True}/\mathrm{False}$
return means "solver converged at the algebraic level, but the FE
error did not plateau within the budget." Treating $S_{\mathrm{solver}}$
alone as "overall verification success" is a misuse of the return
contract.}
...
\subsection{Discretisation-coupled outer-loop tolerance}
...
Quantity~\eqref{eq:dynamic-ftol} governs the staggered drift criteria
of Algorithm~\ref{alg:C} (Sec.~\ref{sec:osgs}) and the plateau target
...
\textbf{Symbol} & \textbf{Field in code} & \textbf{Role / effect} \\
\midrule
$C_{\mathrm{ceil}}$, $C_{\mathrm{sf}}$ & \code{sol.dynamic\_ftol\_ceiling}, \code{\_spatial\_safety\_factor} & Clamps for the discretisation-coupled outer-loop tolerance \eqref{eq:dynamic-ftol}, \S\ref{sec:harness-ftol}; $C_{\mathrm{sf}}$ also sets the per-field dimensionless rate \eqref{eq:per-field-rate}. \\
