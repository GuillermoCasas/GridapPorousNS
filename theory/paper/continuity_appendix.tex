...
% =============================================================================
\section{A continuity proof for the stabilized bilinear form $\BS$}\label{app:Continuity}
...
$C_{\nabla\alpha},C_2,c_J,c_J'$ introduced below, but \emph{not} on the mesh size, nor on
...
porosity variations are resolved by the mesh: there is $C_{\nabla\alpha}\ge0$ such that
\begin{equation}\label{eq:resolved}
  h_K\,\norm{\nabla\alpha}_{L^\infty(K)} \le C_{\nabla\alpha}\,\alpha_{0,K}
  \qquad\forall\,K\in\Th .
\end{equation}
In particular, \amend{the elements being convex,}
$\alpha_{\infty,K}\le \delta_\alpha\,\alpha_{0,K}$ with
$\delta_\alpha\coloneqq 1+C_{\nabla\alpha}$ \amend{(for shape-regular non-convex elements the
same holds with $\delta_\alpha$ enlarged by a constant depending only on the chunkiness
parameter)}, so that
...
$\ba\in C(\overline{\Omega})^d$ \amend{with $\ba|_K\in W^{1,\infty}(K)^d$ for every
$K\in\Th$} (in the application, $\ba$ is the \amend{continuous} finite element velocity
of the previous iteration\amend{, which satisfies this trivially}) and
$\nabla\cdot(\alpha\ba)=0$ in the sense of distributions,
i.e.\ $\int_\Omega \alpha\ba\cdot\nabla\psi \dd\Omega = 0$ for all
$\psi\in W^{1,1}_0(\Omega)$. \amend{Since $\alpha\ba$ is continuous across interior
faces, its distributional divergence carries no face contributions, and the global
identity localises: $\nabla\cdot(\alpha\ba)=0$ a.e.\ in each $K\in\Th$, which is the
form used in \cref{eq:elemibp}.}
\end{assumption}
...
For \ref{P:eps}, \eqref{eq:epscond} on the element $K$ reads
...
$\alpha\le\alpha_K$, $|\ba|\le|\ba|_{\infty,K}$ and \eqref{eq:inverse}.
\end{proof}
...
$E_h = U-U_h$ of \cref{th:Convergence}). The estimate of \cref{lem:continterp} holds with
$\Psi(h)$ in place of its majorant $\psi(h)$: the interpolation norms entering its proof are
mesh $\ell^2${\hyp}norms [the bracketed terms bounded through \eqref{eq:interpzero}], which
its final step only afterwards enlarges to $\psi(h)$; retaining them gives
$\BS(\ba;U-\widehat{U}_h,V_h)\le C\,\Psi(h)\,\triplenorm{V_h}$. By \cref{prop:stability},
consistency \eqref{eq:consistency} and this estimate,
\begin{equation*}
...
\noindent The bound \eqref{eq:convergence} is stated in the sharpest form afforded by the
...
the $\Psi(h)$ statement is the tightest; in particular the local factor $\alpha_K$ makes it
sharper in regions of low porosity, consistently with the robustness analysis of
\cref{sec:Robustness}.
