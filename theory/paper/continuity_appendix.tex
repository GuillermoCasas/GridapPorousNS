...
$E_h = U-U_h$ of \cref{th:Convergence}). By \cref{prop:stability}, consistency
...
\end{equation*}
so $\triplenorm{\Delta_h}\le C\psi(h)$. The triangle inequality and \eqref{eq:Enorm} give
$\triplenorm{U-U_h}\le\triplenorm{U-\widehat{U}_h}+\triplenorm{\Delta_h}\le C\psi(h)$.
\end{proof}

\noindent The bound \eqref{eq:convergence}, which is the porosity{\hyp}weighted form of
the main-text estimate \eqref{eq:ConvergenceResult}, is the sharpest afforded by the
present argument; the coarser bound with the weight $\alpha_K$ replaced by $1$ follows at
once from $\alpha_K\le1$ and matches the form found, without the porosity field,
