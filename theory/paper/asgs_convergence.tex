...
$\BASGS (\ba; U_h,V_h)=B(\ba; U_h,V_h)-\sum_K \langle \mathcal{L}^* V_h,
...
norm here is the ASGS working norm $\triplenorm{\cdot}_{\mathrm{A}}$ of the main text
(\cref{lemma:Stability}); as this appendix concerns only the ASGS method, no ambiguity with the
OSGS norm $\triplenorm{\cdot}_{\mathrm{O}}$ can arise, and we drop the subscript throughout,
writing simply $\triplenorm{\cdot}$. In the expanded form obtained via the identity
$\norm{f}_{\tau_i}=\lVert\tau_i^{1/2}f\rVert_h$ of \cref{eq:TauWeightedProduct}, it reads
...
\end{aligned}
\end{equation}
That $\triplenorm{\cdot}$ is indeed a norm on $\Xh$---the viscous term alone suffices---is the
...
Taking $V_h=U_h$ in $\BASGS (\ba;U_h,U_h)$, the Galerkin term reduces to
...
As $\triplenorm{\cdot}$ is a norm (\cref{lem:definiteness}, established with the norm above),
positivity of the coefficients on the right is coercivity in the working norm; the design
condition on $c_1$ that secures it is detailed in \cref{sec:coercivity}.
...
The stability of the ASGS method is coercivity of $\BASGS $ in the working norm
