...
functional in which the sharp estimate is stated, $\overline{\Psi}(h)$ its coarser $\ell^1$ majorant.
...
\end{equation}
with $\Psi(h)\le\overline{\Psi}(h)$ the porosity{\hyp}weighted $\ell^2$ functional of \eqref{eq:psih};
the coarser $\ell^1$ bound in terms of $\overline{\Psi}(h)$ follows as a corollary.
\end{lemma}
...
\eqref{eq:psih} (and only afterwards, if desired, majorised by $\overline{\Psi}(h)$).
...
the $\ell^1$ bound $C\,\overline{\Psi}(h)$. Second, replacing the elementwise weight $\alpha_K\le1$ by
