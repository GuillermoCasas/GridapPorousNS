...
norm of the main text (the $\triplenorm{\cdot}_{\mathrm{S}}$ of \cref{lemma:Stability}), written
here in the expanded form via the identity $\norm{f}_{\tau_i}=\lVert\tau_i^{1/2}f\rVert_h$ of
\cref{eq:TauWeightedProduct}, is
...
\end{aligned}
\end{equation}
The viscous term already makes $\triplenorm{\cdot}$ a norm: under homogeneous Dirichlet data
the deviatoric{\hyp}symmetric projection controls the full velocity gradient, by an elementary
identity, and the Poincaré inequality then controls the velocity $L^2$ norm, which in turn controls
the pressure gradient through $\alpha\X(V_h)$ and fixes the pressure up to a constant, which
the penalty parameter $\varepsilon>0$ (or the zero-mean constraint when $\varepsilon=0$) uniquely fixes.

\subsubsection{Proof of coercivity (\cref{lemma:Stability})}\label{sec:proof-stability}
We now prove the coercivity result stated in \cref{lemma:Stability} of the main text.
Taking $V_h=U_h$ in $\BS(\ba;U_h,U_h)$, the Galerkin term reduces to
$2\nu\norm{\alpha^{1/2}\ViscProj\nabla\buh}^2+\norm{\sigma^{1/2}\buh}^2+\varepsilon\bigl\lVert p_h \bigr\rVert^2$
by skew-symmetry of the convection term and cancellation of the pressure-divergence pair.
Expanding the stabilization terms yields the residual and divergence squares, a negative
viscous residual square absorbed by the inverse estimate, a reaction square renormalizing
$\sigma$ to the damped weight $\sigmatilde$, and a viscous-reaction cross term bounded by Young's
inequality. Collecting all terms gives
\begin{equation}\label{eq:coerccollect-lemma}
\begin{aligned}
  \BS(\ba;U_h,U_h)
  \ \ge\ &\Bigl(2\bigl(1-\tfrac{2}{\eta}\bigr)-\tfrac{4\Cinva^2}{c_1}\Bigr)\,
  \nu\bigl\lVert\alpha^{1/2}\ViscProj\nabla\buh\bigr\rVert^2
  + \Bigl(1-\tfrac{\eta\Cinva^2}{c_1}\Bigr)\bigl\lVert\sigmatilde^{1/2}\buh\bigr\rVert_h^2 \\
  &+ (1-C_2)\,\varepsilon\bigl\lVert p_h \bigr\rVert^2
...
For every $\bv\in H^1_0(\Omega)^d$,
\begin{equation}\label{eq:devsymidentity}
  \bigl\lVert\ViscProj\nabla\bv\bigr\rVert^2
  = \tfrac12\lVert\nabla\bv\rVert^2 + \Bigl(\tfrac12-\tfrac1d\Bigr)\lVert\nabla\cdot\bv\rVert^2
  \ \ge\ \tfrac12\lVert\nabla\bv\rVert^2 ,
\end{equation}
so that $\lVert\nabla\bv\rVert\le\sqrt2\,\bigl\lVert\ViscProj\nabla\bv\bigr\rVert$; in particular
$\triplenorm{\cdot}$ of \eqref{eq:triplenorm} is a norm on $\Xh$.
\end{lemma}
\begin{proof}
Since $\ViscProj\nabla\bv=\operatorname{sym}\nabla\bv-\tfrac1d(\nabla\cdot\bv)\,\mathbf I$ and the
deviatoric and spherical parts are orthogonal,
$\lVert\ViscProj\nabla\bv\rVert^2=\lVert\operatorname{sym}\nabla\bv\rVert^2-\tfrac1d\lVert\nabla\cdot\bv\rVert^2$.
For $\bv\in H^1_0(\Omega)^d$, integration by parts gives
$\int_\Omega\nabla\bv:\nabla\bv^{\mathsf T}=\int_\Omega(\nabla\cdot\bv)^2$, whence
$\lVert\operatorname{sym}\nabla\bv\rVert^2=\tfrac12\lVert\nabla\bv\rVert^2+\tfrac12\lVert\nabla\cdot\bv\rVert^2$;
combining the two identities yields \eqref{eq:devsymidentity}, and $\tfrac12-\tfrac1d\ge0$ for
$d\in\{2,3\}$. Thus $\triplenorm{V_h}=0$ forces $\ViscProj\nabla\bvh=0$, hence $\nabla\bvh=0$ and
$\bvh=0$ by Poincar\'e; then $\X(V_h)=\nabla p_h$, so the $\tau_1$ term forces $\nabla p_h=0$, and
$p_h=0$ through the $\varepsilon\norm{p_h}^2$ term when $\varepsilon>0$ and through the zero{\hyp}mean
constraint when $\varepsilon=0$.
\end{proof}
...
standing assumptions of \cref{sec:CommonSetting}: the coefficients \cref{H:data} (with
...
constant $\Cinv$, independent of the mesh size, such that for every $\psi_h$ in a
finite{\hyp}dimensional subspace of $H^l(K)$,
\begin{equation}
    \|\psi_h \|_{W^l_p(K)} \le \Cinv\, h^{l-m + d/p - d/q}\, \| \psi_h \|_{W^m_q(K)},
    \label{eq:InverseEstimateFiniteOrderNorm}
\end{equation}
valid for $0 \le m \le l$ and $1\le p,q \le \infty$ (see, e.g.,~\cite{brenner2008mathematical}); we
use it in both its gradient and its $L^\infty$ forms.
...
\eqref{eq:resolved}. \amend{The main text writes $\Cinv$ for this enlarged constant, by an
...
\begin{equation}\label{eq:continuity}
  \bigl\lvert\BS(\ba; U_h,V_h)\bigr\rvert
  \le C\Bigl(
...
\begin{equation}\label{eq:assembly}
  \bigl\lvert\BS(\ba;U,V)\bigr\rvert
  \le C\,\triplenorm{V}\Bigl(\triplenorm{U}
  + \bigl\lVert\alpha_K\tau_2^{1/2}h^{-1}\bu\bigr\rVert_h
  + \bigl\lVert\alpha_K\tau_1^{1/2}h^{-1}p\bigr\rVert_h\Bigr).
\end{equation}
...
$\overline{\Omega}$. Let $\buhat\in\mathcal{V}_{h0}$ be the Lagrange interpolant of $\bu$, and let
$\widehat{p}_h$ be the Lagrange interpolant $I_h p$ of $p$, mean{\hyp}corrected to
$\widehat{p}_h\coloneqq I_h p-\tfrac{1}{|\Omega|}\int_\Omega I_h p$ when $\mathcal{Q}_{h0}$ is the
zero{\hyp}mean subspace (the case $\varepsilon=0$), so that $\widehat{p}_h\in\mathcal{Q}_{h0}$; since $p$
has zero mean, the shift $\tfrac{1}{|\Omega|}\int_\Omega I_h p=\tfrac{1}{|\Omega|}\int_\Omega(I_h p-p)$
is bounded by $\lVert p-I_h p\rVert$ and is invisible in the $\nabla\cdot(\alpha\bvh)$ pairing (its
gradient vanishes and $\int_\Omega\nabla\cdot(\alpha\bvh)=0$ for $\bvh\in H^1_0$) and in the
$\varepsilon=0$ mass term. Set
