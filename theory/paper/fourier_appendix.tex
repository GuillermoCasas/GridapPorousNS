% =============================================================================
% Appendix C: Fourier symbols of the differential operator and the term-by-term
...
\end{equation}
Inserting the velocity block of $\mathbf{K}_{ij}$ from \cref{eq:matrices_stationary_strong_problem}, whose entries are $(\mathbf{K}_{ij})_{ab}=\nu\alpha\,(\delta_{ij}\delta_{ab}+\tfrac13\delta_{ai}\delta_{aj})$ on the diagonal $a=b$ and $(\mathbf{K}_{ij})_{ab}=\nu\alpha\,(\delta_{bi}\delta_{aj}-\tfrac23\delta_{ai}\delta_{bj})$ off it, the contraction collapses to
...
\begin{equation}\label{eq:ftViscEig}
  ...
  \quad(\boldsymbol{v}_\perp\perp\boldsymbol{k}_0,\ \text{multiplicity }d-1),\qquad
...
\end{equation}
which is the first row of \cref{eq:StabilizationParameters}. The squared wavenumber is what makes the identification $c_1\coloneqq|\boldsymbol{k}_0|^2$ consistent downstream.
...
\begin{remark}
All the matrix identities and limits of this appendix --- the contraction \cref{eq:ftViscClosed} and the eigenpairs \cref{eq:ftViscEig}, the convective and coupling symbols \cref{eq:ftConvSymbol,eq:ftBSymbol} with the eigenvalues $\{\pm|\boldsymbol{k}_0|,0^{(d-1)}\}$, the $\sqrt{\lambda}$ pair \cref{eq:ftTauB}, and the assembly \cref{eq:ftTau1,eq:ftTau2} under the scaling \cref{eq:ftLambda} --- are checked symbolically by the companion script \texttt{fourier\_tau\_verification.py} (\textsc{SymPy}), which reports $19/19$ passing checks and is provided with the source of this article.
\end{remark}
