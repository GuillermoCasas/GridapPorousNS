...
%
% 2026-07-29: rem:ftGenericPi (generic viscous projector: the symbol T_Pi; the two-sided
% bounds 1/2|k|^2 I <= T_Pi <= |k|^2 I, hence the [1,2] range of the dropped O(1) factor,
% for the lem:projfamily family ONLY -- for a general (A3)-admissible projector only the
% upper bound survives and the lower one is 2 c_Pi^2) was added.  Its claims are checked in
% proof_verification/sympy/projector_algebra_verification.py, not by the sympy file above.
%
% [known-fragility]  This file is \input by BOTH article.tex (v1) and article_v2.tex (v2),
% so it may cite only labels defined in BOTH.  The labels it uses from outside itself are
% eq:matrices_stationary_strong_problem, eq:StabilizationParameters, eq:Tau1, eq:Tau2,
% eq:CAlpha, eq:TauNavierStokes, eq:DesignConditionOnTauWeak, eq:CDR_equation,
% sec:StabilizationDesign, sec:NumericalExamplesStationaryCase2D, H:projector,
% sec:ViscousProjector, lem:projfamily, lem:projinstances, eq:projmonotone
% -- all present in both builds, the projector ones because they live in the
% main-text subsection sec:ViscousProjector (Sec. 2.1), which is duplicated in
% article.tex and article_v2.tex.  (Until 2026-07-29 they lived in the shared
% appendix asgs_convergence.tex and were cited "of \cref{app:Continuity}".)
%
% 2026-07-29: eq:cPi is now DEFINED HERE, in rem:ftGenericPi.  It used to live in
% the main text (rem:projscope), which was moved out of the paper into the
% standalone note theory/viscous_projector_note/.  This appendix is the only place
% in the paper that needs the ellipticity constant, so it carries its definition.
% =============================================================================
...
Recall the operator \cref{eq:CDR_equation} and its matrices \cref{eq:matrices_stationary_strong_problem}. For the design it is convenient to split the first-order part $\mathbf{A}_{\mathrm{c},i}+\mathbf{A}_{\mathrm{f},i}$ into a purely \emph{convective} block and a symmetric \emph{pressure-gradient/divergence} coupling,
\begin{equation}\label{eq:ftSplit}
  \mathbf{A}_{\mathrm{c},i}(\boldsymbol{w}) + \mathbf{A}_{\mathrm{f},i}
   = \underbrace{\alpha\,\diag(w_i,\dots,w_i,0)}_{\textstyle \mathbf{A}_{\mathrm{v},i}}
   \; + \; \underbrace{\alpha\bigl(\boldsymbol{e}_i\boldsymbol{e}_{n}^{\top}+\boldsymbol{e}_{n}\boldsymbol{e}_i^{\top}\bigr)}_{\textstyle \mathbf{A}_{\mathrm{b},i}},
\end{equation}
...
The closed form \cref{eq:ftViscClosed} is the deviatoric--symmetric instance of a structure that is uniform over the admissible projectors. For any $\ViscProj$ satisfying \cref{H:projector}, written in components as $(\ViscProj\mathbf{M})_{ai}=P_{aibj}M_{bj}$, the velocity block of the viscous symbol is $\widehat{\mathcal{L}}_{\nu}=(2\alpha\nu/h^2)\,\mathbf{T}_{\ViscProj}(\boldsymbol{k}_0)$, where $\mathbf{T}_{\ViscProj}(\boldsymbol{k})_{ab}\coloneqq P_{aibj}k_ik_j$ is the symmetric positive-semidefinite matrix with quadratic form
\begin{equation}\label{eq:ftGenericQuad}
  \boldsymbol{v}^{\mathsf T}\mathbf{T}_{\ViscProj}(\boldsymbol{k})\,\boldsymbol{v}
  = \bigl|\ViscProj(\boldsymbol{v}\otimes\boldsymbol{k})\bigr|^2 ,
\end{equation}
the last identity following from the idempotency and Frobenius self-adjointness of $\ViscProj$. Non-expansiveness then gives $\mathbf{T}_{\ViscProj}(\boldsymbol{k}_0)\le|\boldsymbol{k}_0|^2\,\mathbb{I}_d$, while for every projector whose range contains the deviatoric--symmetric tensors (\cref{lem:projfamily} of \cref{sec:ViscousProjector})---in particular for the whole family of \cref{lem:projinstances}---the pointwise monotonicity \cref{eq:projmonotone} applied to $\boldsymbol{v}\otimes\boldsymbol{k}_0$ gives $\mathbf{T}_{\ViscProj}(\boldsymbol{k}_0)\ge\mathbf{T}_{\DSPi}(\boldsymbol{k}_0)$, and hence, by the eigenvalues \cref{eq:ftViscEig} of the latter, also $\mathbf{T}_{\ViscProj}(\boldsymbol{k}_0)\ge\tfrac12|\boldsymbol{k}_0|^2\,\mathbb{I}_d$. For every projector of that family the spectral radius entering $\tau_{\nu,1}^{-1}$ therefore lies between $\alpha\nu|\boldsymbol{k}_0|^2/h^2$ and $2\alpha\nu|\boldsymbol{k}_0|^2/h^2$: the factor is $2-\tfrac2d$ for $\DSPi$ (the value displayed in \cref{eq:ftTauNu}) and exactly $2$ for $\mathbb{I}$, $\SPi$ and $\DPi$. Dropping this $O(1)$ factor, as is done below \cref{eq:StabilizationParameters}, is thus uniform over the family of \cref{lem:projinstances}, and the parameters \cref{eq:Tau1,eq:Tau2} require no modification. For a general admissible $\ViscProj$ only the upper bound survives verbatim; the lower one becomes $2c_{\ViscProj}^2\,\alpha\nu|\boldsymbol{k}_0|^2/h^2$, where
\begin{equation}\label{eq:cPi}
  c_{\ViscProj}\coloneqq\min\bigl\{\,\bigl|\ViscProj(\boldsymbol{v}\otimes\boldsymbol{k})\bigr|\ :\ |\boldsymbol{v}|=|\boldsymbol{k}|=1\,\bigr\}
\end{equation}
is the ellipticity constant of $\ViscProj$, positive whenever $\ViscProj$ is admissible but not bounded below independently of it (both facts are proved in the companion note on Korn{\hyp}compatible viscous projectors). The discarded factor is then still $O(1)$---it lies in $[2c_{\ViscProj}^2,2]$---and is still absorbed in the algorithmic constant $c_1$, but the window is no longer uniform.
\end{remark}

\subsection{Convective term}\label{sec:ftConvective}
The convective block $\mathbf{A}_{\mathrm{v},i}$ is first order, so
\begin{equation}\label{eq:ftConvSymbol}
  \widehat{\mathcal{L}}_{c}(\boldsymbol{k}_0)
  = \mathrm{i}\,\frac{k_{0,i}}{h}\,\mathbf{A}_{\mathrm{v},i}
...
The block $\mathbf{A}_{\mathrm{b},i}$ couples the $i$-th velocity component to the pressure, and its symbol is the off-diagonal matrix
\begin{equation}\label{eq:ftBSymbol}
  \widehat{\mathcal{L}}_{b}(\boldsymbol{k}_0)
  = \mathrm{i}\,\frac{k_{0,i}}{h}\,\mathbf{A}_{\mathrm{b},i}
...
\begin{equation}\label{eq:ftLambda}
  ...
  \sqrt{\lambda} = \frac{h}{|\boldsymbol{k}_0|\,\tau_{1,\mathrm{NS}}},
\end{equation}
the coupling and porosity-gradient entries become $\tau_{b,1}^{-1}=\alpha/\tau_{1,\mathrm{NS}}$, $\tau_{b,2}^{-1}=c_1\,\alpha\,\tau_{1,\mathrm{NS}}/h^2$ and $\tau_{\nabla\alpha,1}^{-1}=(h/|\boldsymbol{k}_0|)\,|\nabla\alpha|\,\tau_{1,\mathrm{NS}}^{-1}$, with $c_1=|\boldsymbol{k}_0|^2$. For the \emph{momentum} eigenvalue, the viscous and convective entries collapse onto $\alpha\,\tau_{1,\mathrm{NS}}^{-1}$ (ignoring the coefficient of $\tau_{\nu,1}^{-1}$ displayed in \cref{eq:ftTauNu}---which for the default projector $\DSPi$ is $2-\tfrac2d$, equal to $\tfrac43$ for $d=3$ and to $1$ for $d=2$, and is $2$ for $\mathbb{I}$, $\SPi$ and $\DPi$ (\cref{rem:ftGenericPi})---as is done below \cref{eq:StabilizationParameters}, and writing $c_2=|\boldsymbol{k}_0\cos\phi|$ so that $c_2|\boldsymbol{w}|=|\boldsymbol{w}\cdot\boldsymbol{k}_0|$, cf.\ \cref{eq:TauNavierStokes}); the $\tau_{b,1}^{-1}$ entry coincides with this same momentum scale and is therefore the self-referential contribution absorbed into the algorithmic constants (the cancellation noted after \cref{eq:StabilizationParameters}). Adding the porosity-gradient and reaction entries gives
\begin{equation}\label{eq:ftTau1}
  \tau_1^{-1}
   = \Bigl(\alpha + \tfrac{h}{|\boldsymbol{k}_0|}|\nabla\alpha|\Bigr)\tau_{1,\mathrm{NS}}^{-1} + \rho_{\Lambda^{-1}}(\boldsymbol{\sigma})
   = C_\alpha\,\tau_{1,\mathrm{NS}}^{-1} + \rho_{\Lambda^{-1}}(\boldsymbol{\sigma}),
\end{equation}
...
\begin{equation}\label{eq:ftTau2}
  \tau_2^{-1} = \tau_{b,2}^{-1} + \tau_{\sigma,2}^{-1} = \frac{c_1\,\alpha\,\tau_{1,\mathrm{NS}}}{h^2} + \varepsilon
  \quad\Longrightarrow\quad
  \tau_2 = \frac{h^2}{c_1\,\alpha\,\tau_{1,\mathrm{NS}} + \varepsilon\,h^2},
\end{equation}
