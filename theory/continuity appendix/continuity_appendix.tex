...
% ---------------------------------------------------------------------------
% Macros (self-contained; chosen to match the notation of the main text)
% ---------------------------------------------------------------------------
...
\title{Continuity of the stabilized bilinear form for the linearized porous
Navier--Stokes problem:\\ a self-contained proof for the ASGS method}
\author{}
\date{}
...
\begin{abstract}
\noindent We give a complete and self-contained proof of the continuity estimate for the
stabilized (ASGS) bilinear form associated with the linearized porous Navier--Stokes
equations, in the form stated as the continuity lemma of the main text. The proof follows,
step by step, the proof of Lemma~2 in Codina~\cite{codina2001}, adapting every estimate to
the presence of the porosity field $\alpha$ in the differential operator, in the
stabilization parameters and in the working norm. We also record the modifications needed
to obtain the corresponding continuity estimate with respect to the interpolation error,
from which the convergence theorem of the main text follows by the standard
C\'{e}a-type argument of~\cite{codina2001}. Along the way we state precisely all the
hypotheses that the proof requires, and we amend a few minor inaccuracies, both in the main
text (the smallness condition on the penalty parameter $\varepsilon$, the definition of the
broken norm, and the constants of the inverse estimates applied to $\alpha$-weighted
quantities) and in~\cite{codina2001} (a sign in the inter-element term of Eq.~(66) and the
regularity exponent for the pressure in Eq.~(77)); see \cref{sec:amendments}.
\end{abstract}

\section{Setting}\label{sec:setting}

\subsection{The linearized problem and the stabilized bilinear form}
...
for any tensor $\mathbf{T}$.\footnote{The proofs below use only two properties of
$\ViscProj$: that it is a pointwise, constant-coefficient orthogonal projection of
second-order tensors, and that it is Korn-compatible on $H^1_0(\Omega)^d$, i.e.\
$\lVert\nabla\bv\rVert\le C_{\mathrm{K}}\lVert\ViscProj\nabla\bv\rVert$ there. They therefore
hold verbatim for any such operator; the submitted version of the article states them as a
standing assumption and identifies the admissible family. The deviatoric--symmetric choice
kept here is one instance; its specific form is used nowhere below. Definiteness of the
working norm \eqref{eq:triplenorm} needs only the Korn compatibility stated above, and is
established in that generic form in Appendix~C of the article, not here.} Throughout we use the abbreviations
...
\begin{equation}\label{eq:consistency}
  B_{\mathrm{ASGS}}(\ba; U-U_h, V_h) = 0 \qquad \forall\, V_h\in\Xh .
\end{equation}

\subsection{Stabilization parameters and working norm}
...
norm here is the ASGS working norm $\triplenorm{\cdot}_{\mathrm{A}}$ of the main text; as this
note treats only the ASGS method, the subscript is redundant and we drop it, writing simply
$\triplenorm{\cdot}$. It is
...
\Cref{H:jump} is the analogue of condition~(39) in~\cite{codina2001}. By \cref{H:porosity}
...
mild requirement on the advection field already implicit in~\cite{codina2001}.
\end{remark}
...
$\Cinv$ by $\Cinva$ in the conditions on $c_1$ (cf.\ \cref{sec:amendments}).
\end{remark}
...
\section{The continuity estimate}\label{sec:continuity}
...
follows the steps of the proof of Lemma~2 in~\cite{codina2001}.

\medskip\noindent\emph{Step 0: term decomposition.}
Expanding the products in \eqref{eq:Bstab} we obtain
$B_{\mathrm{ASGS}}(\ba;U,V) = \sum_{j=1}^{18} T_j$, with
\begin{align}
...
correspond, in this order, to terms (42)--(59) of~\cite{codina2001}.
...
\begin{align}
  ...
  \label{eq:fiveterms}
\end{align}
which is the porous counterpart of Eq.~(66) of~\cite{codina2001}. (Note that the
inter-element term [III] carries a \emph{minus} sign; in~\cite{codina2001} it is printed
with a plus sign, a harmless typo since the term is estimated in absolute value, cf.\
\cref{sec:amendments}.)
...
$\sigma$ and hence vanish identically when $\sigma=0$. As in~\cite{codina2001}, the jump
condition and the $L^\infty$ inverse estimate are therefore needed only when $\sigma>0$.
\end{remark}

\section{Consequences for the convergence analysis}\label{sec:convergence}
...
\end{equation}
(see \cref{sec:amendments} concerning the exponent $n$). In particular
...
\end{equation}
Standard interpolation theory~\cite{brennerscott,ernguermond} provides, on every $K$,
...
\end{equation}
which are the analogues of Eqs.~(78)--(79) of~\cite{codina2001}. We abbreviate
...
\subsection{Continuity with respect to the interpolation error}
...
\begin{equation}\label{eq:continterp}
  B_{\mathrm{ASGS}}\bigl(\ba;\,U-\widehat{U}_h,\,V_h\bigr)
...
$B_{\mathrm{ASGS}}(\ba;E,V_h)\le C\triplenorm{V_h}\bigl(\triplenorm{E}+C\psi(h)\bigr)$, and
...
there (with the amendments of \cref{rem:winvconst,sec:amendments}).
...
$B_{\mathrm{ASGS}}(\ba;U_h,U_h)\ge C\,\triplenorm{U_h}^2$ for all $U_h\in\Xh$.
\end{proposition}
...
\begin{equation*}
  C\,\triplenorm{E_h}^2
  \le B_{\mathrm{ASGS}}(\ba;E_h,E_h)
  = B_{\mathrm{ASGS}}\bigl(\ba;\widehat{U}_h-U,E_h\bigr)
...
\section{Amendments to the original statements}\label{sec:amendments}

We collect here the small corrections that the above analysis introduces with respect to
the statements in the main text and in~\cite{codina2001}.

\begin{enumerate}[label=\textup{(A\arabic*)},leftmargin=3.2em,itemsep=2pt]
\item\label{am:eps}
\emph{Smallness condition on $\varepsilon$ (main text).} The condition should be stated
in the two-constant form \eqref{eq:epscond},
$\varepsilon\le C_2\,c_1\min_K\{\alpha_K^2\tau_{1,K}/h_K^2\}$ with a fixed $C_2<1$,
rather than as a strict inequality with $C_2=1$. Indeed, with the latter one has
$\varepsilon\tau_{2,K}<\varphi_{1,K}\tau_{1,K}\le1$ but \emph{without a uniform margin}
when $\sigma=0$, so the coercivity of the penalty block,
$\varepsilon(1-\varepsilon\tau_2)\norm{p_h}^2\ge C\varepsilon\norm{p_h}^2$, would not
hold with a constant independent of the data. The same uniform margin
($\varepsilon\tau_2\le C_2<1$) is used here in Steps 3 and 7, and the factor
$\alpha_K^2$ in \eqref{eq:epscond} is what allows the penalty term to be absorbed with
the weight $\alpha_K$ in \eqref{eq:absorb3}. This mirrors Eq.~(34)
of~\cite{codina2001}.
\item\label{am:brokennorm}
\emph{Definition of the broken norm (main text).}
The elementwise norm must be defined as the $\ell^2$ combination
\eqref{eq:brokennotation}, $\norm{\tau^{1/2}f}_h=(\sum_K\tau_K\norm{f}_K^2)^{1/2}$, and
not as the plain sum $\sum_K\norm{f}_{L^2(K)}$; otherwise the working norm
\eqref{eq:triplenorm} and all Cauchy--Schwarz manipulations would be inconsistent.
\item\label{am:hypotheses}
\emph{Hypotheses of the continuity lemma (main text).} Besides positivity of the
algorithmic constants and $\nabla\cdot(\alpha\ba)=0$, the proof requires: the porosity
resolution condition \eqref{eq:resolved} (assumed earlier in the analysis section of the
main text, but to be included among the hypotheses of the lemma); the smallness condition
\eqref{eq:epscond}; when $\sigma>0$, the jump condition \eqref{eq:jumpcond} together with
the $L^\infty$ inverse estimate in \eqref{eq:inverse} (the exact analogue of conditions
(38)--(39) of~\cite{codina2001}); and trace regularity of $\ba$ (\cref{H:advection}).
The trial and test pairs should range over $\Xh=\mathcal{X}_{h0}$ (homogeneous boundary
values), since the boundary faces must drop from the inter-element terms.
\item\label{am:winv}
\emph{Inverse estimates for $\alpha$-weighted quantities (main text).} Since $\alpha$ is
not piecewise polynomial, the inverse estimates applied to
$\nabla\cdot(\alpha\ViscProj\nabla\buh)$ and $\nabla\cdot(\alpha\buh)$ hold with the
constant $\Cinva=\sqrt{d\,\delta_\alpha}\,\Cinv+C_\alpha$ of \cref{lem:winv} rather than
with the bare $\Cinv$; accordingly, the lower bounds imposed on $c_1$ in the stability
lemma should be read with $\Cinva$ in place of $\Cinv$ (cf.\ \cref{rem:winvconst}).
\item\label{am:codinasign}
\emph{Sign of the inter-element term in Eq.~(66) of~\cite{codina2001}.} The elementwise
integration by parts of the convective stabilization term produces the inter-element
contribution with a \emph{negative} sign, cf.\ \eqref{eq:fiveterms}; in~\cite{codina2001}
it is printed with a positive sign. The slip is harmless: the term is subsequently
estimated in absolute value.
\item\label{am:codinadiva}
\emph{The term $\tfrac12(\nabla\cdot\ba)\bv_h$ in~\cite{codina2001}.} In the original
proof (Eqs.~(65) and~(74)) the contribution of $\tfrac12(\nabla\cdot\ba)\bv_h$ to
$X(\bv_h,q_h)$ is absorbed without comment; estimating it as the convective part requires
control of $\norm{\nabla\cdot\ba}_{L^\infty(K)}$ by $|\ba|_{\infty,K}/h_K$, which holds,
e.g., when $\ba$ is a finite element function (by an inverse estimate), but not for
general $\ba\in L^\infty$. In the porous formulation analyzed here the issue does not
arise: under $\nabla\cdot(\alpha\ba)=0$ the convective operator is $\alpha\ba\cdot\nabla$
with no skew term.
\item\label{am:codinaregularity}
\emph{Pressure regularity in Eq.~(77) of~\cite{codina2001}.} The regularity space is
stated with $n\ge k_p$; since the interpolation error
$E_{\mathrm{int}}(p)$ involves $\norm{p}_{H^{k_p+1}}$, it should read $n\ge k_p+1$, as in
\eqref{eq:regularity}.
\end{enumerate}
...
flows},
Computer Methods in Applied Mechanics and Engineering \textbf{190} (2001) 2681--2706.
