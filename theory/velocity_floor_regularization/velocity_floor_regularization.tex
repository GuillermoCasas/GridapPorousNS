...
and \eqref{eq:reg} is a no-op. This is in fact the situation in the convergence studies: the MMS
harnesses set the \emph{mesh-diffusive} weight \texttt{h\_floor\_weight} to $0$ (so that the drag law stays
mesh-independent) while inheriting $u_{\mathrm{base}}=\texttt{u\_base\_floor\_ref}=10^{-4}$ from
\texttt{config/base\_config.json}; the configuration $u_{\mathrm{base}}=0$ with
\texttt{h\_floor\_weight}$=0$ is in fact rejected by the config validator, which requires the velocity
floor to be strictly positive so that the regularization stays $C^1$. Since $u_{\mathrm{base}}=10^{-4}\gg
\eps=10^{-12}$ there, $\eps$ is by the criterion just stated a \emph{no-op} in the convergence studies,
and the errors they report are unaffected by it. It remains a genuine guard for configurations that switch
the base floor off (e.g.\ \texttt{NoRegularization}), which is why it is retained. Keeping $\eps$ a \emph{separate}
