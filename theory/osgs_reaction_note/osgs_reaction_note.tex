...
%  (\DSPi, \Reyn, \Damk, \triplenorm, \dualpairing, \innerprod, ...), so the
...
\newcommand{\DSPi}{\mathsf{P}_{\mathrm{DS}}}  % the paper's \DSPi: its deviatoric--symmetric instance
...
\begin{equation}\label{eq:BS}
  B_\mathrm{S}(\boldsymbol{a};\Uh,\Vh)
...
\begin{equation}\label{eq:reactive-part}
  \mathcal{L}\Uh\big|_{\mathrm{react}} = \sigma\uh,
  \qquad
  \mathcal{L}^*\Vh\big|_{\mathrm{react}} = \sigma\boldsymbol{v}_h .
\end{equation}
%
...
\begin{align}\label{eq:ASGS-expand}
  B_{\mathrm{ASGS}}(\boldsymbol{a};\Uh,\Uh)
  &= 2\nu\big\|\alpha^{1/2}\DSPi\nabla\uh\big\|^2
...
$\|\nabla\!\cdot(\alpha\DSPi\nabla\uh)\|_K\le C_\mathrm{inv}h^{-1}
...
\begin{equation}\label{eq:ASGS-lemma}
  B_{\mathrm{ASGS}}(\boldsymbol{a};\Uh,\Uh)\ \ge\ C\,\triplenorm{\Uh}^2,
  \qquad
  \triplenorm{\Uh}^2
  = \nu\big\|\alpha^{1/2}\DSPi\nabla\uh\big\|^2
...
$B_{\mathrm{ASGS}}(\boldsymbol{a};\Uh,\Uh)\ge c_{\uh}\,\|\uh\|^2+(\text{other nonnegative terms})$
...
estimate
$
  \triplenorm{E_h}\lesssim\sum_K h_K^{-1}\big(\tau_{2,K}^{1/2}\mathrm{E}_{\mathrm{int},K}(\boldsymbol{u})
  +\tau_{1,K}^{1/2}\mathrm{E}_{\mathrm{int},K}(p)\big)
$
