...
\newcommand{\DSPi}{\overset{\scriptscriptstyle{\mathrm{DS}}}{\Pi}}  % the paper's \DSPi; the paper's \DSPi is now the GENERIC projector % paper's Xi
...
\begin{equation}
  ...
  \label{eq:strong}
\end{equation}
with $\DSPi$ the deviatoric--symmetric projection of the velocity gradient and
...
coercivity})
\begin{equation}
  B(\ba;U_h,U_h)=2\nu\big\|\alpha^{1/2}\DSPi\grad\buh\big\|^2
...
+\alpha\grad e_p=\alpha X(U_h)|_{\mathrm{err}}$, with $X(U_h):=\ba\!\cdot\!\grad\buh
