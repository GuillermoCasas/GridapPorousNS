...
Following~\cite{paper}, a linear map $\AnyProj$ on second{-}order tensors, acting pointwise and with
...
\begin{enumerate}[label=\textup{(V\arabic*)},ref=\textup{(V\arabic*)},leftmargin=3.4em]
  \item\label{V:orth} \emph{Orthogonality.} $\AnyProj$ is an orthogonal projection for the Frobenius
  inner product: $\AnyProj\AnyProj=\AnyProj$ and $(\AnyProj\mathbf{A})\!:\!\mathbf{B}=\mathbf{A}\!:\!(\AnyProj\mathbf{B})$
  for all $\mathbf{A},\mathbf{B}$. In particular $|\AnyProj\mathbf{T}|\le|\mathbf{T}|$ pointwise.
...
\begin{equation}\label{eq:family}
  \mathcal{F}\coloneqq\bigl\{\,\IPi,\ \SPi,\ \DPi,\ \DSPi\,\bigr\},
\end{equation}
...
Let $\AnyProj$ satisfy \cref{V:orth} and let its range contain every deviatoric{-}symmetric tensor.
Then $\AnyProj$ satisfies \cref{V:korn} on every domain, with $\CK=\sqrt2$. Every member of
$\mathcal{F}$ qualifies, with sharp constants
$\CK(\IPi)=1$, $\CK(\SPi)=\CK(\DSPi)=\sqrt2$ and $\CK(\DPi)=\sqrt{d/(d-1)}$.
\end{proposition}
...
The answer is that Korn compatibility is a purely algebraic property of $\AnyProj$, independent of
$\Omega$, and detectable on rank{-}one tensors alone.

\begin{definition}[Ellipticity constant]\label{def:cPi}
For a constant{-}coefficient linear map $\AnyProj$ on second{-}order tensors set
\begin{equation}\label{eq:cPi}
  \cP\ \coloneqq\ \min\bigl\{\,\bigl|\AnyProj(\bv\otimes\bk)\bigr|\ :\ |\bv|=|\bk|=1\,\bigr\},
\end{equation}
...
Let $\AnyProj$ satisfy \cref{V:orth}. The following are equivalent:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2.6em]
  \item\label{it:korn} $\AnyProj$ satisfies \cref{eq:PiKorn} on some bounded domain
  $\Omega\subset\mathbb{R}^d$;
  \item\label{it:allomega} $\AnyProj$ satisfies \cref{eq:PiKorn} on \emph{every} domain
  $\Omega\subset\mathbb{R}^d$;
  \item\label{it:cpi} $\cP>0$, i.e.\ the kernel of $\AnyProj$ contains no nonzero rank{-}one tensor.
\end{enumerate}
...
\end{equation}
a rank{-}one tensor for each fixed $\bk$. Since $\AnyProj$ has real, constant coefficients it
commutes with the Fourier transform, and $\bk$ is real, so Plancherel's theorem gives
\begin{equation}\label{eq:plancherel}
\begin{aligned}
  \norm{\AnyProj\nabla\bv}^2
  &= \int_{\mathbb{R}^d}\bigl|\AnyProj\bigl(\widehat{\bv}(\bk)\otimes\bk\bigr)\bigr|^2\dd\bk \\
...
because $\AnyProj$ is real, so that
$\bigl|\AnyProj(\widehat{\bv}\otimes\bk)\bigr|^2
=\bigl|\AnyProj(\boldsymbol{a}\otimes\bk)\bigr|^2+\bigl|\AnyProj(\boldsymbol{b}\otimes\bk)\bigr|^2$
--- and then rescaled, the map $(\bv,\bk)\mapsto\AnyProj(\bv\otimes\bk)$ being bilinear. Hence \cref{eq:PiKorn} holds with
...
$\AnyProj(\bv_0\otimes\bk_0)=0$ for some unit vectors $\bv_0,\bk_0$. Fix
...
\end{equation}
The first term is annihilated by $\AnyProj$ at every point, so
...
$|\AnyProj(\bv_0\otimes\bk_0)|=\cP$ exactly, so the quotient tends to $\cP^{-1}$.
\end{proof}
...
Let $\AnyProj=\IPi-\DSPi$, the projector onto the spherical{-}plus{-}skew part. Its range meets
...
\begin{equation}\label{eq:complement-c}
  \bigl|(\IPi-\DSPi)(\bv\otimes\bk)\bigr|^2
...
\cref{thm:sharp}, $\IPi-\DSPi$ is admissible, with $\CK=\sqrt d$. Note that it is
...
$|\theta|\to\pi/4$. At $\theta=0$ one recovers $\Pi_0=\IPi-\DSPi$ of
...
operator. The viscous block is $(2\alpha\nu/h^2)\,\mathbf{T}_{\AnyProj}(\bk_0)$ with
$\mathbf{T}_{\AnyProj}(\bk)_{ab}\coloneqq P_{aibj}k_ik_j$, whose quadratic form is
\begin{equation}\label{eq:symbol-quad}
  \bv^{\mathsf T}\mathbf{T}_{\AnyProj}(\bk)\,\bv=\bigl|\AnyProj(\bv\otimes\bk)\bigr|^2 ,
\end{equation}
...
\begin{itemize}[leftmargin=1.6em]
  ...
  $\mathbf{T}_{\AnyProj}(\bk_0)\le|\bk_0|^2\,\mathbb{I}_d$ for every admissible $\AnyProj$.
  \item \emph{Lower bound, not uniform.} \Cref{eq:cPi} and \cref{eq:symbol-quad} give
  $\mathbf{T}_{\AnyProj}(\bk_0)\ge\cP^2\,|\bk_0|^2\,\mathbb{I}_d$, and $\cP$ is positive but,
  by \cref{ex:tilted}, not bounded below over the admissible class.
\end{itemize}
For $\AnyProj\in\mathcal{F}$ the optimality in \cref{thm:sharp} turns $\CK\le\sqrt2$ into
$\cP^2\ge\tfrac12$, upgrading the lower bound to
$\mathbf{T}_{\AnyProj}(\bk_0)\ge\tfrac12|\bk_0|^2\mathbb{I}_d$, so the $O(1)$ factor the article
discards when simplifying $\tau_{\nu,1}^{-1}$ lies in the uniform window $[1,2]$ --- it equals
$2-2/d$ for $\DSPi$ and exactly $2$ for $\IPi$, $\SPi$ and $\DPi$. For a general
admissible projector the window degrades to $[2\cP^2,2]$: still $O(1)$ for each fixed $\AnyProj$,
...
\begin{equation}\label{eq:cPi-recipe}
  \cP^2=\min_{|\bk|=1}\ \lambda_{\min}\bigl(\mathbf{T}_{\AnyProj}(\bk)\bigr),
\end{equation}
a $d\times d$ eigenvalue problem minimized over the unit sphere --- \emph{not} a one-shot
eigenvalue computation on the $d^2\times d^2$ matrix of $\AnyProj$, whose spectrum is contained
...
section~C computes
$\cP$ for $\IPi$, $\SPi$, $\DPi$, $\DSPi$, $\IPi-\DSPi$, the skew and the spherical
