...
%  (\DSPi, \Reyn, \Damk, \tauNS, ...), so the equations can be lifted into
...
accumulated by quadrature in a single assembly pass. Here $\DSPi$ is the
...
\begin{equation}\label{eq:DM}
  \DM^k=
    \underbrace{\norm{\alpha\,\bu^k\!\cdot\!\grad\bu^k}}_{\mathrm{convection}}
    +\underbrace{\norm{2\,\divg(\alpha\nu\,\DSPi\grad\bu^k)}}_{\mathrm{viscous}}
...
\begin{itemize}
  ...
  \item \emph{Why the viscous term is unproblematic under Philosophy A.} The weak
    ...
    with $P_1$, $\divg(\alpha\nu\,\DSPi\grad\bu_h)\approx0$ element-wise;
    ...
    factor $C_{\mathrm{inv}}/h$: it overestimates the viscous force on fine meshes
