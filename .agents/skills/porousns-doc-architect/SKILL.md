...
- [ ] **Adjoint Exactness & Divergence Trace**: The theoretical adjoint continuous expansion uses a divergence trace that the code evaluates as `0.5*Δ(v)` in the test space. You MUST document that this trace formally vanishes for strict divergence-free fields, making the implementation an exact theoretical resolution, not an approximation or omission.
