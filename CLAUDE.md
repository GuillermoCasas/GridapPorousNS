...
- **Stabilized OSGS system** — `eq:OSGSProblem` (Eqs. 4.10a–d). ASGS is recovered by setting `π_h = 0`. The staggered linearized iteration is `eq:LinearizedOSGSProblem`; the pseudocode driving the solver (the `asgs_solver.jl` orchestrator + the `osgs_solver.jl` coupled solve) is `alg:StationarySystem`.
